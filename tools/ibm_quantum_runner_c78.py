#!/usr/bin/env python3
"""
LumVorax C78 — HTS 16 problèmes complets IBM réel — lum_local_block — entropie locale vs globale
Backend: ibm_fez 156Q Heron R2 | Cycle: C78
Standard: STANDARD_NAMES.md v4.5 §M-IBM-C78
...
def _log(tag, msg, val=0.0):
    ...
    _forensic.append(e)
    print(f"[C78:{tag}] {msg} val={val:.6f}")

def _save_lum(name, data):
    p = LUM_DIR / f"{name}_C78_{STAMP}.lum"
...
# ════════════════════════════════════════════════════
# C78 — HTS_16_COMPLET
# Objectif : 16 problèmes Hubbard sur ibm_fez — analyse entropie locale
# Modules  : hts_16_real, entropy_local, sign_problem_detect
# ════════════════════════════════════════════════════
...
def run_c78_pipeline(fake=False, shots=1024):
    _log("start", f"══ C78 HTS_16_COMPLET fake={fake} ══", float(shots))
    if not QISKIT_OK:
        _log("error","Qiskit non dispo"); return {}
    backend, fake = ibm_connect(fake)
    if not backend: return {}
    calib = ibm_calib(backend, fake)
    results = {"cycle":"C78","stamp":STAMP,"backend":getattr(backend,"name","fake"),
               "fake":fake,"calibration":calib,"objective":"16 problèmes Hubbard sur ibm_fez — analyse entropie locale"}

    # ── Circuits spécifiques C78 ──

    # Circuit C78 — HTS_16_COMPLET
    ...
    try:
        ...
        _log("C78",f"Transpilé depth {qc.depth()}→{qct.depth()} {n2q} portes2Q fid≈{fid:.4f}",fid)
    ...
    r={"n_q":n_q,"depth_phys":qct.depth() if "qct" in dir() else 0,"gates_2q":n2q,
        ...
        "c65_ref":C78_CALIB_C65,"objective":"16 problèmes Hubbard sur ibm_fez — analyse entropie locale"}
    results["main_circuit"]=r
    _log("C78",f"✅ entropy={entropy:.3f}b fid={fid:.4f} t={et:.1f}s",entropy)
    _save_lum("circuit_c78",r)

    # ── Sauvegardes finales C78
    total_t=time.time()-_t0; results["total_runtime_s"]=round(total_t,2)
    results["forensic_entries"]=len(_forensic)
    rf=FORENSIC_DIR/f"ibm_c78_results_{STAMP}.json"
    with open(rf,"w") as f: json.dump(results,f,indent=2,default=str)
    ff=FORENSIC_DIR/f"ibm_c78_forensic_{STAMP}.json"
    with open(ff,"w") as f: json.dump(_forensic,f,indent=2)
    _save_lum(f"c78_pipeline",{"summary":results,"forensic":len(_forensic)})
    print(f"\n[C78] ══ TERMINÉ en {total_t:.1f}s — {rf.name} ══")
    return results

def main():
    parser=argparse.ArgumentParser(description="LumVorax IBM C78")
    parser.add_argument("--fake",action="store_true"); parser.add_argument("--shots",type=int,default=1024)
    args=parser.parse_args()
    run_c78_pipeline(fake=args.fake or (not IBM_API_KEY),shots=args.shots)
