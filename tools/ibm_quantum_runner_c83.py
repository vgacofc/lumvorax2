#!/usr/bin/env python3
"""
LumVorax C83 — Rapport final consolidé A-Z C67→C83 — toutes métriques — toutes optimisations — tableau comparatif
Backend: ibm_fez 156Q Heron R2 | Cycle: C83
Standard: STANDARD_NAMES.md v4.5 §M-IBM-C83
...
def _log(tag, msg, val=0.0):
    ...
    _forensic.append(e)
    print(f"[C83:{tag}] {msg} val={val:.6f}")

def _save_lum(name, data):
    p = LUM_DIR / f"{name}_C83_{STAMP}.lum"
...
# ════════════════════════════════════════════════════
# C83 — RAPPORT_FINAL_C67_C83
# Objectif : Rapport final LumVorax C67-C83 complet
# Modules  : all_results, comparatif, next_steps
# ════════════════════════════════════════════════════
...
def run_c83_pipeline(fake=False, shots=1024):
    _log("start", f"══ C83 RAPPORT_FINAL_C67_C83 fake={fake} ══", float(shots))
    if not QISKIT_OK:
        _log("error","Qiskit non dispo"); return {}
    backend, fake = ibm_connect(fake)
    if not backend: return {}
    calib = ibm_calib(backend, fake)
    results = {"cycle":"C83","stamp":STAMP,"backend":getattr(backend,"name","fake"),
               "fake":fake,"calibration":calib,"objective":"Rapport final LumVorax C67-C83 complet"}

    # ── Circuits spécifiques C83 ──

    # Circuit C83 — RAPPORT_FINAL_C67_C83
    ...
    try:
        ...
        _log("C83",f"Transpilé depth {qc.depth()}→{qct.depth()} {n2q} portes2Q fid≈{fid:.4f}",fid)
    ...
    r={"n_q":n_q,"depth_phys":qct.depth() if "qct" in dir() else 0,"gates_2q":n2q,
        ...
        "c65_ref":C83_CALIB_C65,"objective":"Rapport final LumVorax C67-C83 complet"}
    results["main_circuit"]=r
    _log("C83",f"✅ entropy={entropy:.3f}b fid={fid:.4f} t={et:.1f}s",entropy)
    _save_lum("circuit_c83",r)

    # ── Sauvegardes finales C83
    total_t=time.time()-_t0; results["total_runtime_s"]=round(total_t,2)
    results["forensic_entries"]=len(_forensic)
    rf=FORENSIC_DIR/f"ibm_c83_results_{STAMP}.json"
    with open(rf,"w") as f: json.dump(results,f,indent=2,default=str)
    ff=FORENSIC_DIR/f"ibm_c83_forensic_{STAMP}.json"
    with open(ff,"w") as f: json.dump(_forensic,f,indent=2)
    _save_lum(f"c83_pipeline",{"summary":results,"forensic":len(_forensic)})
    print(f"\n[C83] ══ TERMINÉ en {total_t:.1f}s — {rf.name} ══")
    return results

def main():
    parser=argparse.ArgumentParser(description="LumVorax IBM C83")
    parser.add_argument("--fake",action="store_true"); parser.add_argument("--shots",type=int,default=1024)
    args=parser.parse_args()
    run_c83_pipeline(fake=args.fake or (not IBM_API_KEY),shots=args.shots)
