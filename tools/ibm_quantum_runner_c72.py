#!/usr/bin/env python3
"""
LumVorax C72 — BTC Grover oracle SHA-256 partiel complet — préfixe 32 bits — MCX réel 156Q
Backend: ibm_fez 156Q Heron R2 | Cycle: C72
Standard: STANDARD_NAMES.md v4.5 §M-IBM-C72
...
def _log(tag, msg, val=0.0):
    ...
    _forensic.append(e)
    print(f"[C72:{tag}] {msg} val={val:.6f}")

def _save_lum(name, data):
    p = LUM_DIR / f"{name}_C72_{STAMP}.lum"
...
# ════════════════════════════════════════════════════
# C72 — BTC_SHA256_ORACLE
# Objectif : Oracle SHA-256 réel sur 156Q ibm_fez
# Modules  : btc_sha256_oracle, mcx_32bit, grover_amplif
# ════════════════════════════════════════════════════
...
def run_c72_pipeline(fake=False, shots=1024):
    _log("start", f"══ C72 BTC_SHA256_ORACLE fake={fake} ══", float(shots))
    if not QISKIT_OK:
        _log("error","Qiskit non dispo"); return {}
    backend, fake = ibm_connect(fake)
    if not backend: return {}
    calib = ibm_calib(backend, fake)
    results = {"cycle":"C72","stamp":STAMP,"backend":getattr(backend,"name","fake"),
               "fake":fake,"calibration":calib,"objective":"Oracle SHA-256 réel sur 156Q ibm_fez"}

    # ── Circuits spécifiques C72 ──

    # Circuit C72 — BTC_SHA256_ORACLE
    ...
    try:
        ...
        _log("C72",f"Transpilé depth {qc.depth()}→{qct.depth()} {n2q} portes2Q fid≈{fid:.4f}",fid)
    ...
    r={"n_q":n_q,"depth_phys":qct.depth() if "qct" in dir() else 0,"gates_2q":n2q,
        ...
        "c65_ref":C72_CALIB_C65,"objective":"Oracle SHA-256 réel sur 156Q ibm_fez"}
    results["main_circuit"]=r
    _log("C72",f"✅ entropy={entropy:.3f}b fid={fid:.4f} t={et:.1f}s",entropy)
    _save_lum("circuit_c72",r)

    # ── Sauvegardes finales C72
    total_t=time.time()-_t0; results["total_runtime_s"]=round(total_t,2)
    results["forensic_entries"]=len(_forensic)
    rf=FORENSIC_DIR/f"ibm_c72_results_{STAMP}.json"
    with open(rf,"w") as f: json.dump(results,f,indent=2,default=str)
    ff=FORENSIC_DIR/f"ibm_c72_forensic_{STAMP}.json"
    with open(ff,"w") as f: json.dump(_forensic,f,indent=2)
    _save_lum(f"c72_pipeline",{"summary":results,"forensic":len(_forensic)})
    print(f"\n[C72] ══ TERMINÉ en {total_t:.1f}s — {rf.name} ══")
    return results

def main():
    parser=argparse.ArgumentParser(description="LumVorax IBM C72")
    parser.add_argument("--fake",action="store_true"); parser.add_argument("--shots",type=int,default=1024)
    args=parser.parse_args()
    run_c72_pipeline(fake=args.fake or (not IBM_API_KEY),shots=args.shots)
