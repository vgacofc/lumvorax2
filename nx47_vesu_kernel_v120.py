...
class UltraAuthentic360Merkle:
    """Streaming Merkle-style log chain for maximal forensic traceability."""
    ...
    def _bit_stats(self, arr: np.ndarray, bit_limit: int) -> Dict[str, Any]:
        a = np.asarray(arr)
        raw = a.tobytes()
...
def slice_adaptive_fusion(volume: np.ndarray) -> np.ndarray:
    """Adaptive fusion map across slices to stabilize signal before feature extraction."""
    z = volume.shape[0]
    if z <= 1:
        return volume[0]
    weights = np.linspace(1.0, 1.4, z, dtype=np.float32)
    weights = weights / (weights.sum() + 1e-6)
    fused = np.tensordot(weights, volume, axes=(0, 0))
...
def hysteresis_topology_3d(prob: np.ndarray, cfg: V120Config) -> np.ndarray:
...
class NX47V120Kernel:
    def __init__(
        self,
        root: Path = Path("/kaggle/input/competitions/vesuvius-challenge-surface-detection"),
        output_dir: Path = Path("/kaggle/working"),
        config: V120Config | None = None,
    ) -> None:
        self.version = "NX47 V120"
        self.root = self._resolve_root(root)
        self.test_dir = self.root / "test_images"
        self.output_dir = output_dir
        self.tmp_dir = output_dir / "tmp_masks_v120"
        self.overlay_dir = output_dir / "overlays_v120"
        self.submission_path = output_dir / "submission.zip"
        self.roadmap_path = output_dir / "v120_roadmap_realtime.json"
        self.logs_path = output_dir / "v120_execution_logs.json"
        self.memory_path = output_dir / "v120_memory_tracker.json"
        self.metadata_path = output_dir / "v120_execution_metadata.json"
        self.ultra_log_path = output_dir / "v120_ultra_authentic_360_merkle.jsonl"

        self.cfg = config or V120Config()
    ...
    def _log_array_ultra(self, stage: str, arr: np.ndarray) -> None:
        ...
        if not self.cfg.ultra_bit_trace_arrays:
            return
        bits = self.ultra._bit_stats(arr, self.cfg.ultra_bit_trace_limit)
        self.log(
            "ARRAY_TRACE",
            stage=stage,
...
if __name__ == "__main__":
    cfg = V120Config(
        top_k_features=int(os.environ.get("V120_TOP_K_FEATURES", "6")),
        target_active_ratio=float(os.environ.get("V120_TARGET_ACTIVE_RATIO", "0.03")),
        full_pixel_trace=os.environ.get("V120_FULL_PIXEL_TRACE", "0") == "1",
        trace_pixel_budget=int(os.environ.get("V120_TRACE_PIXEL_BUDGET", "4000")),
        ultra_console_log=os.environ.get("V120_ULTRA_CONSOLE_LOG", "1") == "1",
        ultra_step_log=os.environ.get("V120_ULTRA_STEP_LOG", "1") == "1",
        ultra_bit_trace_arrays=os.environ.get("V120_ULTRA_BIT_TRACE_ARRAYS", "1") == "1",
        ultra_bit_trace_limit=int(os.environ.get("V120_ULTRA_BIT_TRACE_LIMIT", "32")),
    )
    kernel = NX47V120Kernel(
