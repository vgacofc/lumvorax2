...
def synthesize_with_multi_train_validation(train_pairs, test_input, puzzle_id, 
                                           ...
                                           max_retries: int = 3):
    """
    Synthèse avec validation multi-exemples
    
    CORRECTION SESSION 37:
        ...
        for program in candidates:
            try:
                ...
                # CORRECTION SESSION 37: Validation multi-exemples
                    ...
                    # CORRECTION SESSION 38: Réutiliser programme adapté entre train pairs
                    ...
                    for idx, (train_input, train_output) in enumerate(train_pairs):
                        success, adapted, history = adaptive.adapt_and_retry(
                            puzzle_id=puzzle_id,
                            initial_program=current_program,  # ✅ FIX: Utiliser programme courant
                        ...
                        current_program = adapted  # ✅ FIX: Réutiliser programme adapté
                        ...
                        if idx == 0:
                            ...
                            # Programme adapté réutilisé et validé
                            print(f"      ✓ Programme adapté valide sur train pair {idx+1}/{len(train_pairs)}")
