...
class ParallelWorldFrameEngine:
    ...
    def _predict_world_trajectory(self,
                                  ...
                                  invariants: Any):
        """
        SESSION 55 FIX: Scoring progressif (pas binaire)
        
        Valide le premier train pair avec scoring partiel
        Permet statuts PROMISING/UNCERTAIN pour programmes partiellement corrects
        """
        try:
            # Validation sur PREMIER train pair
            first_input, first_output = train_pairs[0]
