...
# Import advanced pattern detectors
from .advanced_pattern_detectors import AdvancedPatternDetectorManager
...
class PatternMatcher:
    ...
    def __init__(self, verbose: bool = False):
        ...
        # Initialiser manager de détecteurs avancés (pas de paramètre verbose)
        self.advanced_detector = AdvancedPatternDetectorManager()
    ...
    def detect_exact_pattern(
        ...
        # NOUVELLE STRATÉGIE: Collecter TOUS les patterns candidats
        all_candidates = []
        
        # PHASE 1: Tester patterns exacts existants
        exact_detectors = [
            self._detect_bounding_rectangle,      # conf=0.99
            self._detect_l_shape_corner_fill,     # conf=0.97
            self._detect_spaced_row_duplication,  # conf=0.98
        ]
        
        for pattern_detector in exact_detectors:
            pattern = pattern_detector(train_pairs)
            if pattern:
                all_candidates.append(pattern)
                if self.verbose:
                    print(f"[PatternMatcher] Candidat EXACT: {pattern.name} (conf={pattern.confidence})")
        
        # PHASE 2: Tester patterns avancés (6 nouveaux détecteurs)
        # Ces patterns couvrent 64.2% à 26.2% des échecs identifiés
        ...
        for adv_result in advanced_patterns:
            ...
            if adv_result.transformation is None:
                continue
            
            # Convertir PatternDetectionResult en ExactPattern
            exact_pattern = ExactPattern(
                name=adv_result.pattern_name,
                confidence=adv_result.confidence,
                transform_func=adv_result.transformation,
                description=adv_result.description
            )
            all_candidates.append(exact_pattern)
            if self.verbose:
                print(f"[PatternMatcher] Candidat AVANCÉ: {exact_pattern.name} (conf={exact_pattern.confidence})")
        
        # SÉLECTION: Prendre le pattern avec la PLUS HAUTE CONFIANCE
        if not all_candidates:
            return None
        
        best_pattern = max(all_candidates, key=lambda p: p.confidence)
        self.patterns_detected.append(best_pattern)
        
        if self.verbose:
            print(f"[PatternMatcher] ✅ SÉLECTIONNÉ: {best_pattern.name} (conf={best_pattern.confidence})")
