"""
Test MAGEN V30 (TVT) sur environnements synthétiques
====================================================

Compare V30.0.0 (avec TVT) vs V28.2.2 (baseline) sur 7 environnements.

Objectif : Valider que TVT améliore :
- Taux succès global
- Réduction collapse politique
- Efficacité trajectoires
...
def run_single_test(
    ...
    # Modules auxiliaires
    reward_shaper = ProgressiveRewardShaper()
    ...
    for step in range(max_steps):
        ...
        # Update modules
        reward_shaper.update(state, action, reward)
        reputation_system.update(action, reward > 0)
...
def run_comparison_test():
    ...
    avg_steps_v28 = sum(r['steps'] for r in results['v28'] if r['success']) / max(1, success_v28)
    avg_steps_v30 = sum(r['steps'] for r in results['v30'] if r['success']) / max(1, success_v30)
    
    print(f"\nSteps moyens (succès) :")
    print(f"  V28.2.2 : {avg_steps_v28:.1f}")
    print(f"  V30.0.0 : {avg_steps_v30:.1f}")
    ...
    output_file = f"test_v30_synthetic_{timestamp}.json"
