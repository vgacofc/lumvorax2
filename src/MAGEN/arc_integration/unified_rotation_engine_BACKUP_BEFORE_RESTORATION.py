...
class UnifiedPuzzleWrapper:
    ...
    def __init__(self, puzzle_info: UnifiedPuzzleInfo, transformation_engine: Optional[TransformationLearningEngine] = None):
        ...
        # RESTAURATION V26: TransformationLearningEngine pour calcul reward
    ...
    def _step_arc_static(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        RESTAURATION V26: Utilise TransformationLearningEngine pour:
        - Appliquer transformation selon action
        - Calculer reward basé sur similarité avec target
        - Détecter victoire (grille correcte)
        """
        # Si transformation engine disponible, l'utiliser
        if self.transformation_engine is not None:
            # Mapper action vers transformation
            action_map = {
                0: 'identity',
                1: 'rotate_90',
                2: 'rotate_180',
            ...
            action_name = action_map.get(action, 'identity')
            ...
            if result:
                ...
                # Pas de résultat
                output_grid = self.input_grid.copy()
                reward = -0.1
                done = self.current_step >= self.max_steps
        else:
            # Fallback: reward basique (comme avant)
            output_grid = self.input_grid.copy()
            reward = -0.1
            done = self.current_step >= self.max_steps
    ...
    def _step_arcade_2d(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        return np.zeros((64, 64, 3), dtype=np.uint8), reward, done, info
...
class UnifiedRotationEngine:
    ...
    def _play_episode(self, puzzle_info: UnifiedPuzzleInfo, level: int) -> Dict:
        ...
        while not done and steps < max_steps:
            # RESTAURATION V26: Utiliser MAGEN Agent au lieu d'actions aléatoires
            # Pour l'instant: actions semi-intelligentes basées sur transformation engine
            if hasattr(env, 'transformation_engine') and env.transformation_engine is not None:
                # Utiliser transformation engine pour sélectionner action
        ...
        # RESTAURATION V26: Logique victoire CORRECTE
        if puzzle_info.puzzle_type == PuzzleType.ARC_STATIC:
            # ARC: Victoire = reward positif (grille correcte détectée par transformation engine)
            # V26 utilisait: success = error < 0.1 dans validate_and_update
            victory = total_reward > 0.5  # Seuil ajusté pour détecter vraies victoires
        else:
    ...
    def _print_cycle_summary(self, cycle_stats: Dict):
        ...
        print(f"\n🎮 Puzzles joués: {len(cycle_stats['puzzles_played'])}")
