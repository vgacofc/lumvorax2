"""
World Model Simple - Sans dépendances externes (Phase 4.5.1)

Version simplifiée de WorldModel qui n'utilise PAS:
- NetworkX (remplacé par dict Python)
- SciPy (remplacé par flood-fill manuel)

Fonctionnalités identiques, zéro dépendance externe.
...
def flood_fill_component(grid: np.ndarray, start_pos: Tuple[int, int], 
                         value: int, visited: Set[Tuple[int, int]]) -> List[Tuple[int, int]]:
    """
    Flood-fill pour extraire composante connexe
...
def extract_connected_components_simple(grid: np.ndarray) -> List[Entity]:
    """
    Extraire composantes connexes sans SciPy
    
    Utilise flood-fill manuel
    """
    entities = []
    visited = set()
    h, w = grid.shape
    ...
    for value in unique_values:
        ...
        # Trouver toutes composantes de cette valeur
        for y in range(h):
            for x in range(w):
                if grid[y, x] == value and (y, x) not in visited:
                    # Extraire composante
                    pixels = flood_fill_component(grid, (y, x), value, visited)
...
class SimpleGraph:
    """
    Graphe simple sans NetworkX
    
    Remplace nx.Graph()
    """
    def __init__(self):
        self.nodes = {}  # {node_id: node_data}
        self.edges = []  # [(node1, node2, edge_data)]
...
class WorldModel:
    """
    Modèle mental global du monde (VERSION SIMPLE)
    
    Identique à world_model.py mais sans dépendances externes
    """
    ...
    def __init__(self):
        ...
        # Graphe spatial (version simple)
        self.spatial_graph = SimpleGraph()
    ...
    def observe_complete_state(self, grid: np.ndarray) -> Dict[str, Entity]:
        ...
        # Extraire toutes les entités (version simple)
        entities = extract_connected_components_simple(grid)
