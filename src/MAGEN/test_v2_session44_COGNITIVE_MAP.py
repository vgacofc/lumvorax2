...
def _generate_for_classification(train_pairs, test_input, classification):
    ...
    for category, primitives in composer.primitives.items():
        if category in ['geometric', 'scale', 'advanced']:  # ✅ FIX SESSION 45: Inclure 'advanced'
...
def synthesize_with_multi_train_validation(train_pairs, test_input, puzzle_id,
                                           ...
                                           max_retries: int = 3):
    """
    Synthèse avec COGNITIVE STRATEGY MAP
    
    SESSION 44 - RÉVOLUTION COGNITIVE:
    - Génération MASSIVE futurs possibles
    - Représentation spatiale simultanée
    - Détection impasses AVANT exécution
    - Élimination précoce trajectoires mortes
    - Sélection stratégie optimale
    """
    # SESSION 41: Extraction invariants
    ...
    # SESSION 44: Cognitive Strategy Map
    cognitive_map = CognitiveStrategyMap(max_futures=50, beam_width=5)
