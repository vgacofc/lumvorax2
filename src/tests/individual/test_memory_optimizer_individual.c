// Test individuel memory_optimizer - REAL
#include <stdio.h>
#include <time.h>
#include <assert.h>
#include <stdlib.h>
...
static bool test_module_create_destroy(void) {
    ...
    memory_pool_t* pool = memory_pool_create(4096, 64);
    assert(pool != NULL);
    assert(pool->is_initialized == true);
    printf("    ✅ Pool created (size=4096, align=64)\n");
...
static bool test_module_basic_operations(void) {
    ...
    memory_pool_t* pool = memory_pool_create(8192, 64);
    if (pool) {
        void* ptr = memory_pool_alloc(pool, 256);
        if (ptr) {
            printf("    ✅ Allocated 256 bytes\n");
            memory_pool_free(pool, ptr, 256);
...
static bool test_module_stress_100k(void) {
    printf("  Test 3/5: Stress 50 memory_optimizer...\n");
...
static bool test_module_memory_safety(void) {
    ...
    memory_pool_alloc(NULL, 100);
    printf("    ✅ NULL alloc safe\n");
