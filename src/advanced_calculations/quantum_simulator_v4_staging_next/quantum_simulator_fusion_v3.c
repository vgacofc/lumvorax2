...
bool quantum_fusion_v3_run_forensic_benchmark(
    ...
    /* C33 — QF_LOG initialisation run fusion_v3 — STANDARD_NAMES Section I */
    QF_LOG_MODULE_START("fusion_v3", "forensic_benchmark");
    QF_LOG_HW_SAMPLE("fusion_v3");
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:rng_mode", (double)rng_mode);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:scenarios_total", (double)scenarios);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:steps_per_scenario", (double)steps);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:max_qubits_width", (double)max_qubits_width);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:seed", (double)seed);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:used_hw_entropy", used_hw ? 1.0 : 0.0);
    QF_LOG_THREAD_START("fusion_v3", "benchmark_main_thread");
    ...
    for (size_t i = 0; i < scenarios; ++i) {
        ...
        /* C33 — QF_LOG scénario démarré — ring buffer nano (sans I/O bloquante) */
        QF_LOG_NANO("fusion_v3", "scenario_start", (double)i);
        ...
        /* C33 — QF_LOG résultat scénario — CSV LumVorax V4 NEXT */
        QF_LOG_SCENARIO("fusion_v3", i, steps, nx_score, q_score, nx_wins);
        QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:margin",        margin);
        QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:target",        target);
        QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:sigma",         sigma);
        QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:thermal",       thermal);
        QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:nx_err",        nx_err);
        QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:q_err",         q_err);
        QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_scenario:latency_ns",    (double)scenario_latency_ns);
        /* Anomalie : NX perd contre baseline */
        if (margin < 0.0) {
            char desc[128];
            snprintf(desc, sizeof(desc), "scenario_%zu_nx_lost:%s", i, fail_reason);
            QF_LOG_ANOMALY("fusion_v3", desc, margin);
        }
    ...
    /* C33 — QF_LOG summary final run fusion_v3 — INSTRUCTION PERMANENTE 2 */
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:nqubits_simulated", nqubits_simulated);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:nqubits_per_sec",   nqubits_per_sec);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:nx_avg_score",      nx_avg);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:q_avg_score",       q_avg);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:win_rate",          win_rate);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:nx_wins",           (double)nx_wins);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:baseline_wins",     (double)(scenarios - nx_wins));
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:elapsed_ns",        (double)elapsed_ns);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:latency_p50_ns",    (double)p50);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:latency_p95_ns",    (double)p95);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:latency_p99_ns",    (double)p99);
    QF_LOG_MODULE_METRIC("fusion_v3", "fusion_v3_summary:max_qubits_width",  (double)max_qubits_width);
    /* Anomalie si win_rate < 0.5 (NX perd majoritairement) */
    if (win_rate < 0.5) {
        QF_LOG_ANOMALY("fusion_v3", "win_rate_below_50pct", win_rate);
    }
    QF_LOG_HW_SAMPLE("fusion_v3");
    QF_LOG_THREAD_END("fusion_v3", "benchmark_main_thread");
    /* Flush ring buffer nano vers CSV avant fermeture */
    qf_flush_nano_ring();
    QF_LOG_MODULE_END("fusion_v3", "forensic_benchmark", true);
