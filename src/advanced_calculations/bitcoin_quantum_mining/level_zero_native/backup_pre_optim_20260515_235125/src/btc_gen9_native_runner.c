...
typedef struct {
     ...
     */
    /* C255v8g OPTIMISATION #2: Désactiver ctx_id=3 (cache miss -24.5%) */
...
int btc_gen9_reopen_drm(btc_gen9_context_t* ctx) {
    ...
    /* C252v2 ÉTAPE 2 : Fermer/rouvrir DRM (0.5 ms) */
    int old_fd = ctx->drm_fd;
    close(ctx->drm_fd);
    
    ctx->drm_fd = open("/dev/dri/renderD128", O_RDWR);
    if (ctx->drm_fd < 0) {
        LOG_EVENT(ctx, "DRM_REOPEN_FAILED: errno=%d (%s)", errno, strerror(errno));
        return -1;
    }
    LOG_EVENT(ctx, "DRM_REOPEN_FD_OK: old_fd=%d new_fd=%d", old_fd, ctx->drm_fd);
