...
int main(int argc, char** argv) {
    ...
    btc_gen9_context_t* ctx = btc_gen9_init_simple(268435456, 256);
    if (!ctx) {
    ...
    /* Dump batch buffer (avant construction) */
    printf("[FORENSIC] Dump batch buffer (avant construction)...\n");
    ...
    if (batch_map) {
        ...
        printf("[FORENSIC] Batch buffer dumpé (avant, %zu bytes)\n", batch_size);
    }
    
    /* Construire batch buffer */
    printf("\n[TEST] Construction batch buffer...\n");
    if (btc_gen9_build_batch_buffer(ctx) < 0) {
        fprintf(stderr, "ERREUR: Construction batch buffer échouée\n");
        btc_gen9_cleanup(ctx);
        i915_forensic_tracker_cleanup();
        return 1;
    }
    printf("[TEST] Batch buffer construit\n");
    
    /* Dump batch buffer (après construction) */
    printf("[FORENSIC] Dump batch buffer (après construction)...\n");
    if (batch_map) {
        i915_forensic_dump_buffer(batch_bo, batch_map, batch_size);
        printf("[FORENSIC] Batch buffer dumpé (après, %zu bytes)\n", batch_size);
    }
