...
int nx48_alltime_try_update(const char *path,
    ...
    /* [C208-FIX-ALLTIME-1] Logging forensique AVANT toute opération */
    fprintf(stderr, "[C208-ALLTIME-TRACE] try_update lz_new=%d nonce=%u path=%s\n",
            lz_new, nonce_new, path);
    ...
    /* [C208-FIX-ALLTIME-2] Cleanup lock orphelin si processus mort */
    struct stat st;
    if (stat(lockpath, &st) == 0) {
        /* Lock file existe, vérifier s'il est orphelin */
        time_t now = time(NULL);
        if (now - st.st_mtime > 300) {  /* 5 minutes = orphelin probable */
            fprintf(stderr, "[C208-ALLTIME-WARN] Lock orphelin détecté (age=%lds), suppression\n",
    ...
    if (lockfd < 0) {
        fprintf(stderr, "[C208-ALLTIME-ERROR] open lock failed errno=%d\n", errno);
        return -1;
    }
    
    /* [C208-FIX-ALLTIME-3] Écrire PID dans lock file pour traçabilité */
    char pid_str[32];
    snprintf(pid_str, sizeof(pid_str), "%d\n", (int)getpid());
    if (write(lockfd, pid_str, strlen(pid_str)) < 0) {
        fprintf(stderr, "[C208-ALLTIME-WARN] write PID failed errno=%d\n", errno);
    }
    
    if (flock(lockfd, LOCK_EX) != 0) {
        fprintf(stderr, "[C208-ALLTIME-ERROR] flock failed errno=%d\n", errno);
        close(lockfd);
        return -1;
    }
    ...
    if (rc_load < 0) {
        fprintf(stderr, "[C208-ALLTIME-ERROR] load failed rc=%d\n", rc_load);
        flock(lockfd, LOCK_UN);
        close(lockfd);
        return -1;
    }

    /* [C208-FIX-ALLTIME-4] Logging forensique de la comparaison */
    fprintf(stderr, "[C208-ALLTIME-COMPARE] lz_new=%d vs cur.best=%d (condition: %d <= %d = %s)\n",
            lz_new, cur.best_lz_alltime, lz_new, cur.best_lz_alltime,
            (lz_new <= cur.best_lz_alltime) ? "TRUE (skip)" : "FALSE (update)");

    if (lz_new <= cur.best_lz_alltime) {
        /* Pas un nouveau record absolu — on ne touche RIEN (monotonie). */
        fprintf(stderr, "[C208-ALLTIME-SKIP] Pas de nouveau record, skip update\n");
        flock(lockfd, LOCK_UN);
        close(lockfd);
        return 0;
    }
    ...
    /* [C208-FIX-ALLTIME-5] Logging forensique AVANT mise à jour */
    fprintf(stderr, "[C208-ALLTIME-UPDATE] NOUVEAU RECORD ABSOLU: %d -> %d (+%d LZ)\n",
            cur.best_lz_alltime, lz_new, lz_new - cur.best_lz_alltime);
    ...
    /* [C208-FIX-ALLTIME-6] Logging forensique du résultat */
    if (rc == 0) {
        fprintf(stderr, "[C208-ALLTIME-SUCCESS] Fichier %s mis à jour (update_count=%llu)\n",
                path, (unsigned long long)cur.update_count);
    } else {
        fprintf(stderr, "[C208-ALLTIME-ERROR] Échec écriture atomique rc=%d errno=%d\n",
                rc, errno);
    }
    ...
    /* [C208-FIX-ALLTIME-7] Cleanup lock file après succès */
    if (rc == 0) {
        unlink(lockpath);
    }
