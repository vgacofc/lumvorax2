...
#include "../../../neural_network/nx49_biological_neuron.h"  /* C170-NX49-INTEGRATION */
...
_Atomic int nx48_ctrl_batch_size     = 1024; /* Défaut C46 (CPU PT-MC) */
...
/* C255V11 : Gestion dynamique GPU native (i915 DRM) — NX48 contrôle TOUT */
_Atomic uint64_t nx48_ctrl_gpu_mem_available = 0;     /* Mémoire GPU disponible (bytes) */
_Atomic uint64_t nx48_ctrl_gpu_mem_total     = 0;     /* Mémoire GPU totale (bytes) */
_Atomic int      nx48_ctrl_num_contexts      = 2;     /* Nombre contextes GPU (1, 2 ou 3) */

/* C261 : Batch size GPU Gen9 adaptatif [256K, 32M] contrôlé par NX48 SN_BATCH */
_Atomic uint32_t nx48_ctrl_batch_size_gen9   = 8388608;  /* Défaut : 8M nonces (sweet spot) */
_Atomic int      nx48_ctrl_use_thp           = 1;        /* Utiliser THP pour buffers >2MB */
...
void nx48_btc_hw_detect(nx48_btc_state_t* s) {
    ...
    printf("[NX48-HW] CPU:%d threads AVX:%d (%s) SHA-NI:%d | GPU-i915-DRM:%s | RAM:%ldMB\n",
...
void nx48_btc_control_all(nx48_btc_state_t* s) {
    ...
    /* ════════════════════════════════════════════════════════════════════
     * C170-NX49-INTEGRATION : Modulation biologique du contrôle NX48
     * AVANT C170 : Contrôle purement algorithmique (sigmoid, tanh, Adam)
     * APRÈS C170 : Neurone biologique module tous les paramètres via potentiel membrane
     *
     * Mécanisme :
     * 1. Calcul potentiel membrane (Hodgkin-Huxley) : V_m ∈ [-70mV, +40mV]
     * 2. Génération potentiel d'action si V_m > V_threshold (-55mV)
     * 3. Modulation : bio_factor = (V_m - V_rest) / (V_peak - V_rest)
     *                            = (V_m + 70) / 110  ∈ [-0.64, +1.0]
     * 4. Application : Tous paramètres NX48 modulés par bio_factor
     *
     * Impact : Plasticité STDP, canaux ioniques Na⁺/K⁺/Ca²⁺, synapses
     * ════════════════════════════════════════════════════════════════════ */
    if (s->bio_neuron) {
        /* Calcul potentiel membrane (équations Hodgkin-Huxley) */
        double V_m = nx49_neuron_compute_membrane_potential(s->bio_neuron);
        s->last_membrane_potential = V_m;
        
        /* Tentative génération potentiel d'action */
        bool fired = nx49_neuron_fire_action_potential(s->bio_neuron);
        if (fired) {
            s->spike_count++;
            if ((s->spike_count % 100) == 0) {
                printf("[NX48-C170-NX49] ⚡ Spike #%lu | V_m=%.1fmV | Modulation biologique active\n",
                       s->spike_count, V_m);
            }
        }
        
        /* Facteur modulation biologique : normalise V_m sur [-0.64, +1.0]
         * V_rest=-70mV → factor=0.0 (neutre)
         * V_threshold=-55mV → factor=0.136 (légère excitation)
         * V_peak=+40mV → factor=1.0 (excitation maximale)
         * Hyperpolarisation → factor négatif (inhibition) */
        s->bio_modulation_factor = (V_m - NX49_RESTING_POTENTIAL) /
                                   (NX49_ACTION_POTENTIAL_PEAK - NX49_RESTING_POTENTIAL);
        s->bio_modulation_factor = clamp(s->bio_modulation_factor, -0.64, 1.0);
        
        /* Log forensique modulation biologique */
        if ((s->update_count % 100) == 0) {
            FORENSIC_LOG_MODULE_METRIC(BTC_MODULE_NAME, "btc_nx48_c170_bio_V_m", V_m);
            FORENSIC_LOG_MODULE_METRIC(BTC_MODULE_NAME, "btc_nx48_c170_bio_modulation",
                                      s->bio_modulation_factor);
            FORENSIC_LOG_MODULE_METRIC(BTC_MODULE_NAME, "btc_nx48_c170_bio_spikes",
                                      (double)s->spike_count);
        }
    } else {
        s->bio_modulation_factor = 0.0;  /* Pas de modulation si NX49 absent */
    }
    ...
    /* C170-NX49 : Modulation biologique de l'exploration
     * bio_factor > 0 → augmente exploration (neurone excité)
     * bio_factor < 0 → diminue exploration (neurone inhibé) */
    if (s->bio_neuron && fabs(s->bio_modulation_factor) > 0.05) {
        double bio_adjust = s->bio_modulation_factor * 0.10;  /* ±10% max */
        s->exploration_bias = clamp(s->exploration_bias + bio_adjust, 0.05, 0.95);
    }
    ...
    /* ── SN5 : batch size GPU Gen9 ILLIMITÉ adaptatif ── */
    /* C262: NX48 apprend LIBREMENT depuis 256K jusqu'à ILLIMITÉ
     * AVANT C262: Plage fixe [256K, 32M] = LIMITATION ARTIFICIELLE
     * APRÈS C262: Démarrage 256K, progression ILLIMITÉE selon hashrate
     * Stratégie: NX48 double si succès, divise si échec, AUCUNE limite max
     * Objectif: S'adapter à N'IMPORTE QUEL hardware (UHD 620, RTX 4090, etc.) */
    {
        /* C262: NX48 ne décide PAS directement le batch size
         * Il contrôle seulement le FACTEUR de progression via batch_size_scale
         * Le Gen9 Adapter utilise sa logique C258 (256K → ILLIMITÉ)
         * NX48 influence via batch_size_scale ∈ [0.5, 8.0] */
        
        double sn_out = s->subneurons_exec[NX48_SN_BATCH].output;
        
        /* C262: Facteur progression [0.5, 8.0]
         * 0.5 = ralentir progression (hardware faible)
         * 1.0 = progression normale (défaut)
         * 8.0 = accélérer progression (hardware puissant) */
        double scale = 0.5 + sn_out * 7.5;  /* [0.5, 8.0] */
        s->batch_size_scale = scale;
        
        /* C262: NX48 n'écrit PAS nx48_ctrl_batch_size_gen9
         * Le Gen9 Adapter gère TOUT avec sa logique C258 adaptative
         * NX48 observe hashrate et ajuste batch_size_scale pour influencer */
        
        /* Garder batch_size CPU pour compatibilité moteur PT-MC */
        int bsize_cpu = 1024 + (int)(sn_out * 7168.0);  /* [1024, 8192] */
        atomic_store_explicit(&nx48_ctrl_batch_size, bsize_cpu, memory_order_relaxed);
        
        /* Log influence NX48 tous les 100 updates */
        if ((s->update_count % 100) == 0) {
            fprintf(stderr, "[C262-NX48-SN5] 🧠 Facteur progression GPU: %.2fx | sn_out=%.3f\n",
                    scale, sn_out);
        }
    }
...
nx48_btc_state_t* nx48_btc_init(const nx48_btc_config_t* cfg, const char* run_id) {
    ...
    /* C170-NX49-INTEGRATION : Création neurone biologique pyramidal
     * AVANT C170 : Contrôle purement algorithmique (tanh, sigmoid, Adam)
     * APRÈS C170 : Neurone biologique avec soma, dendrites, axon, synapses
     * Type : Pyramidal (cortex, intégration multi-dendritique complexe)
     * Paramètres biologiques : V_rest=-70mV, V_threshold=-55mV, tau_m=20ms
     * Synapses : 100 excitatrices + 25 inhibitrices avec STDP activé
     * Blockchain : Intégration hash Bitcoin comme input synaptique
     * Impact : Plasticité biologique réelle, potentiels d'action, modulation ionique */
    s->bio_neuron = nx49_neuron_create("pyramidal");
    s->last_membrane_potential = NX49_RESTING_POTENTIAL;  // -70.0 mV
    s->spike_count = 0;
    s->bio_modulation_factor = 0.0;
    
    if (!s->bio_neuron) {
        fprintf(stderr, "[NX48-BTC-C170] ⚠️  ERREUR : Échec création neurone NX49 biologique\n");
    } else {
        printf("[NX48-BTC-C170] ✅ NX49 neurone biologique activé (pyramidal)\n");
        printf("[NX48-BTC-C170]    └─ Soma: %.1fµm | Dendrites: %d | Axon: %.0fµm\n",
               NX49_SOMA_DIAMETER, 100, NX49_AXON_LENGTH);
        printf("[NX48-BTC-C170]    └─ V_rest: %.1fmV | V_threshold: %.1fmV | Synapses: 125\n",
               NX49_RESTING_POTENTIAL, NX49_THRESHOLD_POTENTIAL);
        FORENSIC_LOG_MODULE_METRIC(BTC_MODULE_NAME, "btc_nx48_c170_nx49_active", 1.0);
    }
    ...
    double                   hashrate_mhs)
{
    ...
    /* ════════════════════════════════════════════════════════════════
     * C172-NX49-INTEGRATION : Connexion neurone biologique au pipeline
     * AVANT C172 : NX49 créé mais inerte (0 spikes, aucun input synaptique)
     * APRÈS C172 : Inputs synaptiques connectés aux métriques mining réelles
     *
     * Inputs synaptiques (3 dendrites excitatrices) :
     *   1. Hashrate normalisé [0,1] : hashrate_mhs / 100.0
     *   2. Leading zeros normalisé [0,1] : best_leading_zeros / 64.0
     *   3. Nouveau record binaire {0,1} : (best_nonce != s->best_nonce)
     *
     * Modulation biologique :
     *   - Potentiel membranaire V_membrane influence exploration_bias
     *   - Spikes (potentiels d'action) déclenchent boost delta_nonce
     *   - STDP (plasticité synaptique) apprend patterns temporels
     *
     * Impact attendu :
     *   - Adaptation biologique réelle aux patterns de mining
     *   - Modulation ionique (Na+, K+, Ca2+) influence décisions NX48
    ...
    if (s->bio_neuron) {
        ...
        /* Logging forensique périodique (toutes les 100 updates) */
        if (s->update_count % 100 == 0) {
            FORENSIC_LOG_MODULE_METRIC(BTC_MODULE_NAME, "btc_nx49_V_membrane", V_mem);
            FORENSIC_LOG_MODULE_METRIC(BTC_MODULE_NAME, "btc_nx49_spike_count",
                                       (double)s->spike_count);
            FORENSIC_LOG_MODULE_METRIC(BTC_MODULE_NAME, "btc_nx49_bio_modulation",
                                       s->bio_modulation_factor);
        }
    }
...
#undef NX48_SAFE_RESET_SCALAR
    ...
    /* C172-FIX-BATCH : Limite stricte batch_size_scale à 1.0 (262,144 nonces max)
     * AVANT C172 : [0.5, 8.0] → explosion jusqu'à 2,097,152 nonces (8.0 × 262k)
     * APRÈS C172 : [0.5, 1.0] → max 262,144 nonces (respecte capacité Intel UHD 620)
     * Raison : GPU max_work_group_size=256, max_work_item_sizes=256
     * Impact : Évite CL_INVALID_WORK_GROUP_SIZE et activation GPU réelle */
...
void nx48_btc_destroy(nx48_btc_state_t* s) {
    if (s) {
        /* C170-NX49-INTEGRATION : Libération neurone biologique */
        if (s->bio_neuron) {
            printf("[NX48-BTC-C170] Libération neurone NX49 (spikes=%lu)\n", s->spike_count);
            nx49_neuron_destroy(s->bio_neuron);
            s->bio_neuron = NULL;
        }
        LV_FREE(s);
    }
