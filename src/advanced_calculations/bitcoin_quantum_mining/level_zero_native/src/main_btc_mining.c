...
int main(int argc, char* argv[]) {
    ...
    /* C255v8n CORRECTION #4 : Mode TESTNET3 par défaut pour soumission réelle */
    strncpy(cfg.run_mode,  "TESTNET3",                   sizeof(cfg.run_mode)-1);
    ...
    /* C171-AC-02-FIX : Initialiser logger GPU asynchrone AVANT nx48_btc_init
     * Buffer capacity = 262144 (1 batch GPU complet) */
    printf("[C171-GPU-ASYNC] Initialisation logger GPU asynchrone (buffer 262k)...\n");
    if (btc_gpu_async_logger_init(262144) != 0) {
        fprintf(stderr, "[C171-GPU-ASYNC] ERREUR : Échec init logger GPU asynchrone\n");
        return 1;
    }
    printf("[C171-GPU-ASYNC] Logger GPU asynchrone actif\n");
    ...
    if (!lv_sha256_self_test()) {
        ...
        btc_gpu_async_logger_shutdown();  /* C171 : Flush buffer GPU avant exit */
    ...
    /* C171-AC-02-FIX : Shutdown logger GPU asynchrone (flush buffer + join thread) */
    printf("[C171-GPU-ASYNC] Arrêt logger GPU asynchrone (pending: %zu entries)...\n",
           btc_gpu_async_logger_pending());
    btc_gpu_async_logger_shutdown();
    printf("[C171-GPU-ASYNC] Logger GPU asynchrone arrêté proprement\n");
