/*
 * TEST C508 - BATCH BUFFER OPENCL EXACT
 *
 * STRATÉGIE: Reproduire EXACTEMENT le batch buffer OpenCL capturé (320 bytes)
 * Source: logs/forensic/batch_intercept_c506_47561.log
 *
 * DIFFÉRENCES IDENTIFIÉES vs C503:
 * 1. PIPELINE_SELECT complet (20 DWORDs) - MANQUANT dans C503
 * 2. MEDIA_VFE_STATE exact OpenCL - DIFFÉRENT dans C503
 * 3. STATE_BASE_ADDRESS complet (19 DWORDs) - SIMPLIFIÉ dans C503
 * 4. PIPE_CONTROL avec flags exacts - DIFFÉRENT dans C503
 * 5. MEDIA_INTERFACE_DESCRIPTOR_LOAD exact - PRÉSENT mais différent dans C503
 * 6. Commande 0x11000001 (UNKNOWN) - ABSENTE dans C503
 *
...
static uint32_t build_opencl_exact_batch(uint32_t *batch, 
    ...
    printf("  Construction batch OpenCL EXACT (320 bytes)...\n");
    ...
    // ============================================================================
    // Padding (5 x 2 DWORDs) - Offset 0x011c
    // ============================================================================
    for (int i = 0; i < 5; i++) {
        batch[offset++] = 0x00000000;
        batch[offset++] = 0x00000000;
    ...
    printf("  ✓ Batch OpenCL EXACT construit: %u bytes\n", offset*4);
...
int main(void) {
    ...
    printf("\n=== RÉSULTATS GPU NATIF (BATCH OPENCL EXACT) ===\n");
...
// Made with Bob - C508 Batch OpenCL Exact