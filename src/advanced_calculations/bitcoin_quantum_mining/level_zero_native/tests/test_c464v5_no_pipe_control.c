/*
 * TEST C464v5 - SANS PIPE_CONTROL
 * Hypothèse: PIPE_CONTROL cause le GPU HANG
 * 
 * Contenu ABSOLU minimal:
 * - PIPELINE_SELECT (GPGPU mode)
 * - MI_BATCH_BUFFER_END
 * 
 * Si ce test réussit → PIPE_CONTROL est le problème
 * Si ce test échoue → PIPELINE_SELECT GPGPU est le problème
 */
...
int main(void) {
    FORENSIC_LOG("🚀 TEST C464v5 - SANS PIPE_CONTROL");
    ...
    FORENSIC_LOG("⚡ Soumission (SANS PIPE_CONTROL)...");
    ...
    FORENSIC_LOG("🎉 TEST C464v5 RÉUSSI - PIPE_CONTROL était le problème!");
