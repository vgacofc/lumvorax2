// TEST C564 - CORRECTION ROOT CAUSE #75: BATCH_LEN ALIGNEMENT 8 BYTES
// Objectif: Respecter validation kernel i915 ligne 2213
// Référence: C563 - ROOT CAUSE #75 identifié (batch_len & 0x7 != 0)
// Stratégie: Padding batch 228 → 232 bytes (alignement 8)
// Validation: if ((exec->batch_start_offset | exec->batch_len) & 0x7) return -EINVAL;
...
int main() {
    printf("=== TEST C564 - CORRECTION ROOT CAUSE #75 ===\n");
    printf("ROOT CAUSE #75: batch_len=228 non aligné 8 bytes (228 & 0x7 = 0x4)\n");
    printf("CORRECTION: Padding batch 228 → 232 bytes (alignement 8)\n");
    printf("VALIDATION KERNEL: (batch_start_offset | batch_len) & 0x7 == 0\n\n");
    ...
    printf("STRATÉGIE C564: Output → Heap → Batch (batch_len aligné 8)\n");
    ...
    // ✅ CORRECTION ROOT CAUSE #75: Padding alignement 8 bytes
    uint32_t batch_raw_len = idx * 4;  // 228 bytes
    uint32_t batch_complet_len = (batch_raw_len + 7) & ~7;  // Align 8: 232 bytes
    ...
    printf("[89%%] ✓ Batch complet: %u DWords (%u bytes, aligné 8) ✅\n", idx, batch_complet_len);
    printf("       Raw: %u bytes → Padded: %u bytes (padding: %u)\n",
           batch_raw_len, batch_complet_len, batch_complet_len - batch_raw_len);
    
    // === PASS 2: EXÉCUTION GPU FINALE AVEC BATCH ALIGNÉ 8 ===
    printf("\n=== PASS 2: EXÉCUTION GPU FINALE (90%%) ===\n");
    printf("STRATÉGIE: batch_len=%u (aligné 8 bytes) ✅\n", batch_complet_len);
    
    // ✅ CORRECTION ROOT CAUSE #74: Output NON-PINNED (relocation auto)
    struct drm_i915_gem_exec_object2 objects_pass2[3] = {
        {
            .handle = output_handle,
            .flags = EXEC_OBJECT_SUPPORTS_48B_ADDRESS | EXEC_OBJECT_WRITE,
            .offset = 0  // ❌ PAS PINNED - i915 allouera GTT automatiquement
        },
        {
            .handle = heap_handle,
            .flags = EXEC_OBJECT_SUPPORTS_48B_ADDRESS | EXEC_OBJECT_PINNED,
            .offset = heap_gtt  // ✅ NON-NULL (0x1000)
        },
        {
            .handle = batch_handle,
            .flags = EXEC_OBJECT_SUPPORTS_48B_ADDRESS | EXEC_OBJECT_PINNED,
            .offset = batch_gtt  // ✅ NON-NULL (0x5000), DERNIER = batch par défaut
        }
    };
    ...
    struct drm_i915_gem_execbuffer2 execbuf_pass2 = {
        ...
        .buffer_count = 3,
        .batch_start_offset = 0,  // ✅ Début du buffer batch (dernier élément)
        .batch_len = batch_complet_len,
        .flags = I915_EXEC_RENDER,  // ✅ SANS BATCH_FIRST (défaut = dernier)
        .rsvd1 = context_id,
        .rsvd2 = 0
    };
    ...
    fprintf(stderr, "[DEBUG]   output: NON-PINNED (relocation auto) ✅\n");
    fprintf(stderr, "[DEBUG]   flags = I915_EXEC_RENDER ✅\n");
    fprintf(stderr, "[DEBUG]   batch_len = %u bytes (aligné 8) ✅\n", batch_complet_len);
    fprintf(stderr, "[DEBUG]   Validation: (%u | %u) & 0x7 = 0x%x\n",
            0, batch_complet_len, (0 | batch_complet_len) & 0x7);
    
    printf("[92%%] Soumission PASS 2 (batch_len aligné 8)...\n");
