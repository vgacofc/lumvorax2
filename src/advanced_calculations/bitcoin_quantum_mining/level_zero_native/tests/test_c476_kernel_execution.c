/*
 * TEST C476 - KERNEL EXECUTION RÉEL + OUTPUT
 * 
 * Base: test_c475v3_simple_gpgpu.c (FONCTIONNE ✅)
 * Ajout: Kernel bytecode + Interface Descriptor + Output buffer
 * 
 * Objectif: Premier 0xDEADBEEF écrit par GPU natif i915
 * 
 * Séquence complète Beignet:
 * 1. Batch null_state (960 DWORDs)
 * 2. PIPE_CONTROL
 * 3. PIPELINE_SELECT GPGPU
 * 4. MEDIA_VFE_STATE
 * 5. CURBE_LOAD
 * 6. IDRT_LOAD (avec relocation kernel)
 * 7. GPGPU_WALKER
 * 8. MEDIA_STATE_FLUSH
 * 9. PIPE_CONTROL
 */
...
int main(void) {
    FORENSIC_LOG("🚀 TEST C476 - KERNEL EXECUTION RÉEL");
    ...
    /* Créer buffers: batch + 4 relocs + kernel + output + idrt */
    ...
    FORENSIC_LOG("✅ Buffers créés: batch=%u kernel=%u output=%u idrt=%u relocs=[%u,%u,%u,%u]",
                 batch_bo, kernel_bo, output_bo, idrt_bo,
    ...
    FORENSIC_LOG("✅ Kernel ISA Gen9 RÉEL écrit: %zu bytes", minimal_kernel_size);
    FORENSIC_LOG("📊 État: 35%%");
    ...
    struct drm_i915_gem_relocation_entry relocs[NUM_RELOCS + 2];
    ...
    // Reloc IDRT kernel_start_pointer (dans IDRT buffer)
    ...
    // Reloc IDRT_LOAD offset (dans batch)
    ...
    FORENSIC_LOG("✅ Relocations préparées: %zu total", NUM_RELOCS + 2);
    FORENSIC_LOG("📊 État: 75%%");
    
    /* EXECBUFFER2 */
    struct drm_i915_gem_exec_object2 exec_objects[8];
    ...
    exec_objects[6].relocs_ptr = (uint64_t)&relocs[NUM_RELOCS];
    exec_objects[7].handle = batch_bo;
    exec_objects[7].relocation_count = NUM_RELOCS + 1;
    exec_objects[7].relocs_ptr = (uint64_t)relocs;
