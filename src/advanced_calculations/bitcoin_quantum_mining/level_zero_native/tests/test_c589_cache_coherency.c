/*
 * TEST C589 - CACHE COHERENCY MANUELLE (ROOT CAUSE #129)
 *
 * BASE: C584 fonctionnel (EXECBUFFER2 SUCCESS, GPU exec 694ms)
 * AJOUT: Cache flush manuel CPU↔GPU (has_snoop=no)
 *
 * ROOT CAUSE #129: has_snoop=no → coherency manuelle requise
 * SOLUTION: clflush() + PIPE_CONTROL DC_FLUSH + memory barrier
 * 
...
int main(void) {
    ...
    printf("🚀 TEST C589 - CACHE COHERENCY MANUELLE (ROOT CAUSE #129)\n");
    printf("═══════════════════════════════════════════════════════════════════════\n");
    printf("BASE: C584 fonctionnel (EXECBUFFER2 SUCCESS)\n");
    printf("AJOUT: Cache flush manuel (clflush + PIPE_CONTROL DC_FLUSH)\n");
    printf("═══════════════════════════════════════════════════════════════════════\n\n");
    ...
    // 7.6. CACHE FLUSH CPU→GPU (ROOT CAUSE #129)
    printf("  🔄 Flushing CPU cache to GPU...\n");
    for (size_t i = 0; i < HEAP_SIZE; i += 64) {
        __builtin_ia32_clflush((void *)(heap + i));
    }
    __sync_synchronize();  // Memory barrier
    printf("  ✅ CPU cache flushed (%zu cache lines)\n", HEAP_SIZE / 64);
    ...
    /* 6. PIPE_CONTROL (flush final + DC_FLUSH) - 6 DWords */
    printf("    [%2d] PIPE_CONTROL (flush final + DC_FLUSH)\n", offset);
    batch[offset++] = GEN9_PIPE_CONTROL | (6 - 2);
    batch[offset++] = 0x00100000 | 0x00000800;  // CS_STALL + DC_FLUSH (ROOT CAUSE #129)
    batch[offset++] = 0;
    batch[offset++] = 0;
    batch[offset++] = 0;
    batch[offset++] = 0;
