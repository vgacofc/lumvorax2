...
// Objectif: Corriger EINVAL en augmentant batch PASS 1 à 16 bytes (4 DWords)
// Référence: RAPPORT_C557 - ROOT CAUSE #65 identifié
...
int main() {
    printf("=== TEST C558 - CORRECTION ROOT CAUSE #65 ===\n");
    printf("ROOT CAUSE #65: Batch PASS 1 trop court (12 bytes)\n");
    printf("CORRECTION: Batch PASS 1 = 16 bytes (4 DWords minimum)\n\n");
    ...
    // === CRÉATION BUFFERS (ORDRE CRITIQUE) ===
    printf("\n=== CRÉATION BUFFERS (5%%) ===\n");
    printf("STRATÉGIE: Créer heap EN DERNIER pour éviter GTT=0x0\n");
    
    // Batch buffer (4KB) - PREMIER
    ...
    printf("[7%%] ✓ Batch BO créé (handle=%u) - Premier buffer\n", batch_handle);
    
    // Output buffer (4KB) - DEUXIÈME
    ...
    printf("[9%%] ✓ Output BO créé (handle=%u) - Deuxième buffer\n", output_handle);
    
    // Heap buffer (16KB) - DERNIER (pour éviter GTT=0x0)
    ...
    printf("[11%%] ✓ Heap BO créé (handle=%u) - DERNIER (évite GTT=0x0)\n", heap_handle);
    ...
    // === PASS 1: BATCH MINIMAL 16 BYTES (CORRECTION C558) ===
    printf("\n=== PASS 1: BATCH MINIMAL 16 BYTES (31%%) ===\n");
    
    uint32_t idx = 0;
    batch[idx++] = 0x00000000;  // MI_NOOP
    batch[idx++] = 0x00000000;  // MI_NOOP
    batch[idx++] = 0x00000000;  // MI_NOOP (4ème DWord pour atteindre 16 bytes)
    batch[idx++] = 0x0A000000;  // MI_BATCH_BUFFER_END
    
    uint32_t batch_minimal_len = idx * 4;
    printf("[33%%] ✓ Batch minimal: %u DWords (%u bytes) ✅ CORRIGÉ\n", idx, batch_minimal_len);
    ...
    fprintf(stderr, "[DEBUG]   heap_handle = %u\n", heap_handle);
    fprintf(stderr, "[DEBUG]   output_handle = %u\n", output_handle);
    fprintf(stderr, "[DEBUG]   batch_handle = %u\n", batch_handle);
    fprintf(stderr, "[DEBUG] Batch content:\n");
    for (uint32_t i = 0; i < idx; i++) {
        fprintf(stderr, "[DEBUG]   batch[%u] = 0x%08x\n", i, batch[i]);
    }
    
    // Objets PASS 1 (ORDRE: batch, output, heap)
    ...
    fprintf(stderr, "[DEBUG] execbuf_pass1:\n");
    fprintf(stderr, "[DEBUG]   buffers_ptr = 0x%016llx\n", (unsigned long long)execbuf_pass1.buffers_ptr);
    fprintf(stderr, "[DEBUG]   buffer_count = %u\n", execbuf_pass1.buffer_count);
    fprintf(stderr, "[DEBUG]   batch_len = %u\n", execbuf_pass1.batch_len);
    fprintf(stderr, "[DEBUG]   flags = 0x%llx\n", execbuf_pass1.flags);
    fprintf(stderr, "[DEBUG]   rsvd1 (ctx) = %llu\n", execbuf_pass1.rsvd1);
    
    printf("[35%%] Soumission PASS 1 (16 bytes, context %u)...\n", context_id);
    if (ioctl(fd, DRM_IOCTL_I915_GEM_EXECBUFFER2, &execbuf_pass1) < 0) {
        fprintf(stderr, "❌ PASS 1 ÉCHEC: %s (errno=%d)\n", strerror(errno), errno);
        fprintf(stderr, "[DEBUG] Vérifier dmesg pour erreurs kernel i915\n");
        close(fd);
        return 1;
    }
    
    // Lire adresses GTT PASS 1 (ordre: batch, output, heap)
    uint64_t batch_gtt = objects_pass1[0].offset;
    uint64_t output_gtt = objects_pass1[1].offset;
    uint64_t heap_gtt_pass1 = objects_pass1[2].offset;
    
    printf("[40%%] ✓ PASS 1 RÉUSSI! 🎉\n");
    printf("       Batch:  0x%016lx\n", batch_gtt);
    printf("       Output: 0x%016lx\n", output_gtt);
    printf("       Heap:   0x%016lx %s\n", heap_gtt_pass1,
           heap_gtt_pass1 == 0 ? "⚠️ NULL!" : "✅ NON-NULL");
    
    fprintf(stderr, "[DEBUG] GTT Addresses PASS 1:\n");
    fprintf(stderr, "[DEBUG]   batch_gtt = 0x%016lx\n", batch_gtt);
    fprintf(stderr, "[DEBUG]   output_gtt = 0x%016lx\n", output_gtt);
    fprintf(stderr, "[DEBUG]   heap_gtt_pass1 = 0x%016lx\n", heap_gtt_pass1);
    
    // === PATCHER AVEC GTT PASS 1 (temporaire) ===
    printf("\n=== PATCHER STRUCTURES TEMPORAIRE (45%%) ===\n");
    
    // Patcher Surface State
    surface_state[1] = (uint32_t)(output_gtt & 0xFFFFFFFF);
    surface_state[2] = (uint32_t)(output_gtt >> 32);
    printf("[47%%] ✓ Surface State patché\n");
    
    // Patcher IDRT (temporaire avec heap_gtt_pass1)
    uint64_t kernel_gtt_temp = heap_gtt_pass1 + 0x100;
    idrt[0] = (uint32_t)(kernel_gtt_temp & 0xFFFFFFFF);
    idrt[1] = (uint32_t)(kernel_gtt_temp >> 32);
    printf("[49%%] ✓ IDRT patché temporaire (kernel @ 0x%016lx)\n", kernel_gtt_temp);
    
    // === PASS 2: OBTENIR NOUVELLE ADRESSE HEAP ===
    printf("\n=== PASS 2A: OBTENIR ADRESSE HEAP VALIDE (51%%) ===\n");
    
    struct drm_i915_gem_exec_object2 objects_pass2a[3] = {
        {
            .handle = batch_handle,
    ...
    struct drm_i915_gem_execbuffer2 execbuf_pass2a = {
        .buffers_ptr = (uint64_t)objects_pass2a,
        .buffer_count = 3,
        .batch_start_offset = 0,
        .batch_len = 16,
        .flags = I915_EXEC_RENDER,
    ...
    printf("[53%%] Soumission PASS 2A (obtenir heap_gtt valide)...\n");
    if (ioctl(fd, DRM_IOCTL_I915_GEM_EXECBUFFER2, &execbuf_pass2a) < 0) {
        fprintf(stderr, "❌ PASS 2A ÉCHEC: %s (errno=%d)\n", strerror(errno), errno);
        close(fd);
        return 1;
    }
    
    // Lire nouvelle adresse heap (index 2)
    uint64_t heap_gtt = objects_pass2a[2].offset;
    
    printf("[55%%] ✓ PASS 2A RÉUSSI!\n");
    printf("       Batch:  0x%016lx\n", objects_pass2a[0].offset);
    printf("       Output: 0x%016lx\n", objects_pass2a[1].offset);
    printf("       Heap:   0x%016lx %s\n", heap_gtt,
           heap_gtt == 0 ? "❌ ENCORE NULL!" : "✅ NON-NULL!");
    
    fprintf(stderr, "[DEBUG] GTT Addresses PASS 2A:\n");
    fprintf(stderr, "[DEBUG]   batch_gtt = 0x%016lx\n", objects_pass2a[0].offset);
    fprintf(stderr, "[DEBUG]   output_gtt = 0x%016lx\n", objects_pass2a[1].offset);
    fprintf(stderr, "[DEBUG]   heap_gtt = 0x%016lx\n", heap_gtt);
    
    if (heap_gtt == 0) {
        fprintf(stderr, "❌ ERREUR CRITIQUE: heap_gtt=0x0 même après réorganisation!\n");
        fprintf(stderr, "   i915 alloue TOUJOURS 0x0 pour certains buffers\n");
        fprintf(stderr, "   Solution alternative nécessaire (soft-pin, MMAP_GTT, etc.)\n");
        close(fd);
        return 1;
    }
    
    // Re-patcher structures avec heap_gtt valide
    printf("\n=== RE-PATCHER STRUCTURES (57%%) ===\n");
    
    uint64_t kernel_gtt = heap_gtt + 0x100;
    idrt[0] = (uint32_t)(kernel_gtt & 0xFFFFFFFF);
    idrt[1] = (uint32_t)(kernel_gtt >> 32);
    printf("[59%%] ✓ IDRT re-patché (kernel @ 0x%016lx)\n", kernel_gtt);
    
    // === CONSTRUCTION BATCH COMPLET ===
    printf("\n=== CONSTRUCTION BATCH COMPLET (61%%) ===\n");
    ...
    printf("[65%%] ✓ STATE_BASE_ADDRESS (heap GTT=0x%016lx)\n", heap_gtt);
    ...
    printf("[75%%] ✓ MEDIA_INTERFACE_DESCRIPTOR_LOAD\n");
    ...
    batch[idx++] = 0x00000000;  // Reserved
    printf("[80%%] ✓ GPGPU_WALKER (1 work-item)\n");
    
    // MEDIA_STATE_FLUSH
    batch[idx++] = 0x70040000;
    printf("[85%%] ✓ MEDIA_STATE_FLUSH\n");
    ...
    printf("[87%%] ✓ PIPE_CONTROL\n");
    
    // MI_BATCH_BUFFER_END
    batch[idx++] = 0x0A000000;
    printf("[89%%] ✓ MI_BATCH_BUFFER_END\n");
    
    uint32_t batch_complet_len = idx * 4;
    printf("[90%%] ✓ Batch complet: %u DWords (%u bytes)\n", idx, batch_complet_len);
    
    // === PASS 2B: EXÉCUTION GPU FINALE ===
    printf("\n=== PASS 2B: EXÉCUTION GPU FINALE (91%%) ===\n");
    
    struct drm_i915_gem_exec_object2 objects_pass2b[3] = {
        {
            .handle = batch_handle,
            .flags = EXEC_OBJECT_SUPPORTS_48B_ADDRESS,  // ✅ Pas PINNED (batch_gtt=0x0)
            .offset = 0  // Laisser i915 choisir
        },
        {
            .handle = output_handle,
        ...
        },
        {
            .handle = heap_handle,
            .flags = EXEC_OBJECT_SUPPORTS_48B_ADDRESS | EXEC_OBJECT_PINNED,
            .offset = heap_gtt  // ✅ Adresse NON-NULL obtenue en PASS 2A
        }
    };
    
    struct drm_i915_gem_execbuffer2 execbuf_pass2b = {
        .buffers_ptr = (uint64_t)objects_pass2b,
        .buffer_count = 3,
        .batch_start_offset = 0,
    ...
    printf("[93%%] Soumission PASS 2B (batch complet avec heap_gtt=0x%016lx)...\n", heap_gtt);
    if (ioctl(fd, DRM_IOCTL_I915_GEM_EXECBUFFER2, &execbuf_pass2b) < 0) {
        fprintf(stderr, "❌ PASS 2B ÉCHEC: %s (errno=%d)\n", strerror(errno), errno);
        close(fd);
        return 1;
    }
    printf("[94%%] ✓ PASS 2B RÉUSSI - Batch soumis au GPU!\n");
    ...
    if (success) {
        ...
        printf("✓ ROOT CAUSE #65 RÉSOLU (batch 16 bytes minimum)\n");
        printf("✓ ROOT CAUSE #64 RÉSOLU (context i915)\n");
        printf("✓ ROOT CAUSE #63 RÉSOLU (stratégie 2-PASS)\n");
        printf("✓ PASS 1: Batch minimal 16 bytes → GTT addresses\n");
        printf("✓ PASS 2: Batch complet → GPU execution\n");
        ...
        printf("Analyse: GPU exécuté mais résultat incorrect\n");
        printf("Prochaine étape: Vérifier kernel ISA ou synchronisation\n");
        return 1;
    }
}

// Made with Bob - Cycle C558