/*
 * TEST C513 - FORCER RÉSOLUTION GTT (OPTION A)
 *
 * ROOT CAUSES #11, #12, #13, #14 CORRIGÉES + #15:
 ...
 * - #14: batch_len corrigé (MI_BATCH_BUFFER_END avant relocations)
 * - #15: Adresses GTT forcées avec drm_intel_bo_pin()
 *
 * STRATÉGIE OPTION A:
 * 1. Forcer allocation GTT avec drm_intel_bo_pin()
 * 2. Vérifier offset64 non nul
 * 3. Configurer IDRT avec adresses GTT VALIDES
 *
...
int main(void) {
    ...
    printf("\n");
    printf("╔════════════════════════════════════════════════════════════════╗\n");
    printf("║  TEST C513 - FORCER RÉSOLUTION GTT (OPTION A)                ║\n");
    printf("╚════════════════════════════════════════════════════════════════╝\n");
    printf("\n");
    printf("ROOT CAUSES #11, #12, #13, #14, #15 CORRIGÉES!\n");
    printf("Stratégie: Forcer allocation GTT avec drm_intel_bo_pin()\n");
    printf("Objectif: Adresses GTT VALIDES dans IDRT\n");
    printf("\n");
    ...
    // 6. FORCER RÉSOLUTION GTT (ROOT CAUSE #15 FIX - OPTION A!)
    printf("\n[6/10] FORCER résolution adresses GTT avec drm_intel_bo_pin()...\n");
    
    // Forcer allocation GTT pour kernel_bo
    printf("  🔧 Forçage GTT kernel_bo...\n");
    int ret_pin = drm_intel_bo_pin(kernel_bo, 4096);
    if (ret_pin != 0) {
        fprintf(stderr, "    ⚠ drm_intel_bo_pin(kernel_bo) failed: %d\n", ret_pin);
    } else {
        printf("    ✓ kernel_bo pinned\n");
    }
    
    // Forcer allocation GTT pour output_bo
    printf("  🔧 Forçage GTT output_bo...\n");
    ret_pin = drm_intel_bo_pin(output_bo, 4096);
    if (ret_pin != 0) {
        fprintf(stderr, "    ⚠ drm_intel_bo_pin(output_bo) failed: %d\n", ret_pin);
    } else {
        printf("    ✓ output_bo pinned\n");
    }
    
    // Forcer allocation GTT pour idrt_bo
    printf("  🔧 Forçage GTT idrt_bo...\n");
    ret_pin = drm_intel_bo_pin(idrt_bo, 4096);
    if (ret_pin != 0) {
        fprintf(stderr, "    ⚠ drm_intel_bo_pin(idrt_bo) failed: %d\n", ret_pin);
    } else {
        printf("    ✓ idrt_bo pinned\n");
    }
    
    // Vérifier adresses GTT résolues
    printf("\n  ✓ Vérification adresses GTT après pin:\n");
    printf("    kernel_bo: GTT 0x%016lx %s\n", (unsigned long)kernel_bo->offset64,
           kernel_bo->offset64 != 0 ? "✓ VALIDE" : "❌ NULLE");
    printf("    output_bo: GTT 0x%016lx %s\n", (unsigned long)output_bo->offset64,
           output_bo->offset64 != 0 ? "✓ VALIDE" : "❌ NULLE");
    printf("    idrt_bo:   GTT 0x%016lx %s\n", (unsigned long)idrt_bo->offset64,
           idrt_bo->offset64 != 0 ? "✓ VALIDE" : "❌ NULLE");
    
    // Configuration Interface Descriptor avec adresses GTT FORCÉES
    printf("\n  🔧 Configuration IDRT avec adresses GTT FORCÉES...\n");
    drm_intel_bo_map(idrt_bo, 1);
    uint32_t *idrt = (uint32_t*)idrt_bo->virtual;
    memset(idrt, 0, 4096);
    
    // IDRT structure (8 DWORDs) - Utiliser adresses GTT FORCÉES
    ...
    printf("  ✓✓✓ IDRT prêt avec adresses GTT FORCÉES (OPTION A)! ✓✓✓\n");
  ...
  // IDRT déjà configuré à l'étape 6 avec adresses GTT forcées
...
// Made with Bob - C513 FORCER GTT (OPTION A - ROOT CAUSES #11-#15 FIX)