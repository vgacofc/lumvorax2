/*
 * TEST C479 - RELOCATIONS CONTIGUËS (CORRECTION C477v2)
 *
 * Base: test_c477v2_binding_table_fixed.c
 * Correction: Relocations organisées en tableaux contigus par buffer
 *
 * ROOT CAUSE C477v2: relocs_ptr doit pointer vers tableau contigu
 * - batch_bo avait relocs[0-3] + relocs[7] (NON contigu!)
 * - Solution: Créer tableaux séparés contigus
 *
 * Objectif: Premier 0xDEADBEEF GPU natif i915 ✅
 */
...
int main() {
    FORENSIC_LOG("🚀 TEST C479 - RELOCATIONS CONTIGUËS (CORRECTION C477v2)");
    ...
    /* Configurer relocations CONTIGUËS par buffer */
    
    /* Relocations batch_bo (5 relocations contiguës) */
    ...
    for (int i = 0; i < 4; i++) {
        batch_relocs[i].target_handle = reloc_bo[i];
        batch_relocs[i].delta = 0;
        batch_relocs[i].offset = gen9_null_state_relocs[i].offset;
        batch_relocs[i].presumed_offset = 0;
        batch_relocs[i].read_domains = I915_GEM_DOMAIN_RENDER;
        batch_relocs[i].write_domain = I915_GEM_DOMAIN_RENDER;
    }
    
    /* Reloc 4: Batch IDRT_LOAD → idrt_bo */
    batch_relocs[4].target_handle = idrt_bo;
    batch_relocs[4].delta = 0;
    batch_relocs[4].offset = idrt_load_offset * 4 + 12;
    batch_relocs[4].presumed_offset = 0;
    batch_relocs[4].read_domains = I915_GEM_DOMAIN_INSTRUCTION;
    batch_relocs[4].write_domain = 0;
    
    /* Relocations idrt_bo (2 relocations contiguës) */
    struct drm_i915_gem_relocation_entry idrt_relocs[2];
    memset(idrt_relocs, 0, sizeof(idrt_relocs));
    
    /* Reloc 0: IDRT.desc0 → kernel_bo */
    idrt_relocs[0].target_handle = kernel_bo;
    idrt_relocs[0].delta = 0;
    idrt_relocs[0].offset = offsetof(gen8_interface_descriptor_t, desc0);
    idrt_relocs[0].presumed_offset = 0;
    idrt_relocs[0].read_domains = I915_GEM_DOMAIN_INSTRUCTION;
    idrt_relocs[0].write_domain = 0;
    
    /* Reloc 1: IDRT.desc4 → surface_heap_bo */
    idrt_relocs[1].target_handle = surface_heap_bo;
    idrt_relocs[1].delta = 0;
    idrt_relocs[1].offset = offsetof(gen8_interface_descriptor_t, desc4);
    idrt_relocs[1].presumed_offset = 0;
    idrt_relocs[1].read_domains = I915_GEM_DOMAIN_INSTRUCTION;
    idrt_relocs[1].write_domain = 0;
    
    /* Relocations surface_heap_bo (1 relocation) */
    struct drm_i915_gem_relocation_entry surface_relocs[1];
    memset(surface_relocs, 0, sizeof(surface_relocs));
    
    /* Reloc 0: surface_state.ss1 → output_bo */
    surface_relocs[0].target_handle = output_bo;
    surface_relocs[0].delta = 0;
    surface_relocs[0].offset = offsetof(surface_heap_t, surface) + 0 * sizeof(gen7_surface_state_t) + offsetof(gen7_surface_state_t, ss1);
    surface_relocs[0].presumed_offset = 0;
    surface_relocs[0].read_domains = I915_GEM_DOMAIN_RENDER;
    surface_relocs[0].write_domain = I915_GEM_DOMAIN_RENDER;
    
    FORENSIC_LOG("✅ 8 relocations configurées (3 tableaux contigus)");
    ...
    exec_objects[6].relocs_ptr = (uintptr_t)idrt_relocs;  // Tableau contigu
    ...
    exec_objects[7].relocs_ptr = (uintptr_t)surface_relocs;  // Tableau contigu
    ...
    exec_objects[8].relocs_ptr = (uintptr_t)batch_relocs;  // Tableau contigu
