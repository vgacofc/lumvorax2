...
int main(void) {
    printf("\n╔════════════════════════════════════════════════════════════════╗\n");
    printf("║  TEST C524 - IDRT COPIE EXACTE OPENCL (ROOT CAUSE #34)      ║\n");
    printf("╚════════════════════════════════════════════════════════════════╝\n\n");
    printf("ROOT CAUSE #34: IDRT DW6 manquant (Barrier + CURBE Length)\n");
    printf("Solution: Copie byte-par-byte IDRT OpenCL buffer[8]\n\n");
    ...
    // [8/16] COPIE EXACTE IDRT OpenCL buffer[8]
    printf("[8/16] COPIE EXACTE IDRT OpenCL buffer[8]...\n");
    
    // IDRT OpenCL exact (32 bytes = 8 DWORDs) depuis hexdump buffer[8]
    // Offset 0x00: 00 00 04 70 00 00 00 00  → DW0-1: Kernel pointer (sera patché)
    // Offset 0x08: 20 00 00 00 00 00 00 00  → DW2: Sampler = 0x20
    // Offset 0x0C: 0d 00 05 71 00 00 00 00  → DW3: 0x7105000d
    // Offset 0x10: 40 06 00 00 00 d0 fd ff  → DW4: 0x00000640, DW5: 0xfffdd000
    // Offset 0x14: 07 00 00 80 00 00 00 00  → DW6: 0x80000007, DW7: 0x00000000
    
    const uint32_t opencl_idrt[8] = {
        0x70040000,  // DW0: Kernel pointer LOW (sera patché avec notre kernel)
        0x00000000,  // DW1: Kernel pointer HIGH (sera patché)
        0x00000020,  // DW2: Sampler State Pointer
        0x7105000d,  // DW3: BTI pointer (0x0d) + flags (0x710500)
        0x00000640,  // DW4: Entry Count (1) + Thread Config
        0xfffdd000,  // DW5: Thread Group Size
        0x80000007,  // DW6: Barrier Enable (bit 31) + CURBE Read Length (7)
    ...
    printf("  ✓ IDRT copié depuis OpenCL buffer[8]\n");
    printf("  ✓ Kernel pointer patché: 0x%016lx\n", kernel_addr);
    printf("  ✓ IDRT DW2 (Sampler): 0x%08X\n", opencl_idrt[2]);
    printf("  ✓ IDRT DW3 (BTI+flags): 0x%08X\n", opencl_idrt[3]);
    printf("  ✓ IDRT DW4 (Config): 0x%08X\n", opencl_idrt[4]);
    printf("  ✓ IDRT DW5 (Thread Size): 0x%08X\n", opencl_idrt[5]);
    printf("  ✓ IDRT DW6 (Barrier+CURBE): 0x%08X ⭐ CRITIQUE!\n\n", opencl_idrt[6]);
    ...
    printf("  🔧 PASS 2 - IDRT COPIE EXACTE OPENCL:\n");
    printf("     IDRT: Copie byte-par-byte buffer[8]\n");
    printf("     DW6: 0x80000007 (Barrier + CURBE Length) ⭐\n");
    printf("     CURBE Data: Arguments kernel chargés\n");
    printf("     Surface State: Format Gen9 exact\n");
    printf("     Binding Table: Offset 0x1A0\n\n");
    ...
    if (output[0] == 0x12345678) {
        ...
        printf("✅ IDRT COPIE EXACTE OPENCL VALIDÉE!\n");
        printf("✅ ROOT CAUSE #34 RÉSOLU (DW6 Barrier+CURBE)!\n");
