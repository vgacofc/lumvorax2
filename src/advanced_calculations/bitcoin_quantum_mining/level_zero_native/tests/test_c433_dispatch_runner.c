/*
 * TEST C433 - RUNNER VALIDATION STORE avec DISPATCH COMPLET
 * Objectif: Valider pipeline GPU → RAM → CPU avec kernel minimal + batch buffer complet
 * 
 * Ce runner teste:
 * - Chargement kernel Gen9 ISA
 * - Création surface state output buffer
 * - BATCH BUFFER COMPLET (copié depuis btc_gen9_native_runner.c)
 * - Dispatch 256 threads avec MEDIA_CURBE_LOAD + MEDIA_STATE_FLUSH
...
static void configure_interface_descriptor(gpu_context_t* ctx) {
    ...
    *output_addr_ptr = 0;  /* Sera patché par relocation */
    ...
    idd[0] = 0x00000000;  /* Kernel Start Pointer (relocation) */
...
static int build_batch_buffer(gpu_context_t* ctx) {
    ...
    /* C462 FIX CRITIQUE: Ajouter relocation Kernel Start Pointer dans IDD[0]! */
    /* Relocations */
    struct drm_i915_gem_relocation_entry relocs[7];  /* 6 → 7 relocations */
    ...
    /* Kernel relocation (STATE_BASE_ADDRESS Instruction Base) */
    ...
    /* C462 FIX: Kernel Start Pointer relocation (IDD[0] dans DSH) */
    /* IDD est à offset 64 bytes dans DSH, IDD[0] = Kernel Start Pointer */
    relocs[4].target_handle = ctx->kernel_bo;
    relocs[4].offset = 64;  /* DSH offset 64 bytes = IDD[0] */
    ...
    relocs[5].offset = 32;  /* DSH offset 32 bytes = CURBE r8.0 */
    ...
    relocs[6].target_handle = ctx->output_bo;
    relocs[6].offset = 96;  /* SSH offset 96 bytes = Surface State DW8 (Base Address) */
    ...
    exec_objects[3].relocation_count = 2;  /* C462: 1 → 2 (CURBE + IDD Kernel Start) */
    exec_objects[3].relocs_ptr = (uint64_t)&relocs[4];  /* relocs[4]=IDD, relocs[5]=CURBE */
...
int main(int argc, char** argv) {
    FORENSIC_LOG("🚀 TEST C433 - VALIDATION STORE avec DISPATCH COMPLET");
