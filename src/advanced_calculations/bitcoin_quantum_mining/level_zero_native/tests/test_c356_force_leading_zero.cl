/*
 * Test C356 - Force Leading Zero Detection
 * 
 * Ce kernel teste TOUS les nonces d'une plage et écrit dans un buffer
 * les leading_zeros de chaque nonce pour analyse statistique.
 * 
 * Objectif: Prouver que count_leading_zeros() fonctionne correctement
 */
...
/* C356 FIX: Count leading zeros from MSB (hash[7]) to LSB (hash[0]) */
...
__kernel void test_force_leading_zero_c356(
    __global uint *results,     /* [nonce, leading_zeros, hash[8]] × N */
    uint start_nonce,
    uint count
) {
    ...
    /* Previous block hash + Merkle root = zeros */
    for (int i = 4; i < 68; i++) header[i] = 0x00;
    ...
    /* Write results: [nonce, leading_zeros, hash[8]] */
    uint offset = gid * 10;
