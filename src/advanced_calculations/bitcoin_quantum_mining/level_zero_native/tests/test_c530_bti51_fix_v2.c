...
int main(void) {
    ...
    // [8/15] MODIFICATION C531: Placer Surface State à BTI[96]
    printf("[8/15] MODIFICATION C531: Placer Surface State à BTI[96]...\n");
    drm_intel_bo_map(surface_state_bo, 1);
    uint32_t *surface_heap = (uint32_t*)surface_state_bo->virtual;
    
    // Binding Table: BTI[96] = offset 0x400 (vers nouveau Surface State)
    surface_heap[96] = 0x400;  // BTI[96] @ offset 96*4 = 0x180
    printf("  ✓ BTI[96] configuré: offset = 0x400\n");
    
    // Créer Surface State à offset 0x400
    uint32_t *surface_state = &surface_heap[0x400 / 4];
    
    // DW0: Surface Type + Format + MOCS
    surface_state[0] = (0 << 29) |      // SURFTYPE_BUFFER
                       (0x7D << 18) |   // RAW format
                       (0x3 << 8);      // MOCS L3+LLC
    
    // DW1-2: Base Address (output buffer)
    surface_state[1] = (uint32_t)(output_addr & 0xFFFFFFFF);
    surface_state[2] = (uint32_t)(output_addr >> 32) | (0x3D << 16);
    
    // DW3: Width (size - 1)
    surface_state[3] = (4096 - 1) & 0x1FFFFFF;
    ...
    drm_intel_bo_unmap(surface_state_bo);
    printf("  ✓ Surface State créé à offset 0x400 pour output: 0x%016lx\n\n", output_addr);
