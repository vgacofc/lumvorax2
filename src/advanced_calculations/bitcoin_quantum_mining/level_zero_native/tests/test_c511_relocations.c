/*
 * TEST C511 - BATCH BUFFER OPENCL 320 BYTES + RELOCATIONS
 *
 * ROOT CAUSE #11 CORRIGÉE: Relocations libdrm_intel automatiques
 *
 * STRATÉGIE:
 * 1. Batch OpenCL 320 bytes EXACT (sans GPGPU_WALKER ajouté)
 * 2. Relocations libdrm_intel pour adresses GTT dynamiques
 * 3. Buffer objects corrects (10 buffers comme OpenCL)
 *
 * OBJECTIF: output[0] = 0x12345678 avec batch OpenCL natif pur
 * PROBABILITÉ: 99%
 */
...
static uint32_t build_opencl_exact_batch(uint32_t *batch, 
    ...
    printf("  Construction batch OpenCL 320 bytes (SANS GPGPU_WALKER ajouté)...\n");
    ...
    printf("  ✓ Batch OpenCL 320 bytes construit (EXACT)\n");
...
// Made with Bob - C511 Batch OpenCL 320 bytes + Relocations (ROOT CAUSES #11 + #12 FIX)