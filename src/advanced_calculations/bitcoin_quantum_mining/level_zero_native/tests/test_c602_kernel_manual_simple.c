 ...
 *
 * Test avec kernel ISA manuel minimal pour isoler le problème:
 * - Kernel sans dépendances CURBE
 * - Écriture directe 0x12345678 via MOV immédiat
 * - Adresse GTT hardcodée (0x100000)
 *
 * Objectif: Déterminer si le problème est dans l'ISA kernel ou l'infrastructure
 *
 * Date: 2026-05-27
 * Cycle: C602
 * ROOT CAUSE: #151 (Test kernel manuel pour isoler problème ISA)
 */
...
// ROOT CAUSE #151: Utiliser kernel manuel ultra-simple
#include "test_c601_kernel_manual_simple.h"
...
int test_1_configuration_complete(void) {
    printf("\n");
    printf("╔════════════════════════════════════════════════════════════╗\n");
    printf("║  TEST 1 - KERNEL MANUEL ULTRA-SIMPLE                     ║\n");
    printf("╚════════════════════════════════════════════════════════════╝\n\n");
    ...
    // ROOT CAUSE #151: Copier kernel manuel ultra-simple
    auto_buffer_t *kernel_buf = &gen.buffers[BUFFER_TYPE_KERNEL];
    memcpy(kernel_buf->cpu_addr, kernel_c601_manual_simple, kernel_c601_manual_simple_size);
    printf("✓ Kernel MANUEL SIMPLE copié (%zu bytes)\n", kernel_c601_manual_simple_size);
    printf("  → Écriture directe 0x12345678 sans dépendances CURBE\n");
...
int test_2_generation_batch_kernel(void) {
    ...
    // Vérifier PIPE_CONTROL (ROOT CAUSE #154 FIX) - C604
    assert(batch[47] == 0x7A000004);
    printf("✓ PIPE_CONTROL: 0x%08x (ROOT CAUSE #154 FIX)\n", batch[47]);
    
    // Vérifier MI_BATCH_BUFFER_END (maintenant à position 53 avec PIPE_CONTROL)
    assert(batch[53] == 0x05000000);
    printf("✓ MI_BATCH_BUFFER_END: 0x%08x\n", batch[53]);
    printf("✓ Batch total: 54 DWords (216 bytes) avec PIPE_CONTROL\n");
...
int main(void) {
    ...
    printf("╔════════════════════════════════════════════════════════════╗\n");
    printf("║                                                            ║\n");
    printf("║  TEST C602 - KERNEL MANUEL ULTRA-SIMPLE                   ║\n");
    printf("║  Mini Compute Runtime i915 DRM Natif                      ║\n");
    printf("║                                                            ║\n");
    printf("║  Objectif: Isoler problème ISA vs infrastructure          ║\n");
    printf("║  ROOT CAUSE #151: Test kernel sans dépendances CURBE      ║\n");
    printf("║                                                            ║\n");
    printf("╚════════════════════════════════════════════════════════════╝\n");
