...
__kernel void btc_sha256_mining_gen9(
    ...
    /* Utiliser local memory pour trouver le meilleur dans le workgroup */
    ...
    /* C353 FIX: SANS ATOMICS - Comparaison simple thread par thread */
    ...
    /* C353 FIX: SANS ATOMICS - Écriture simple si meilleur */
    if (lid == 0) {
        /* Lire valeur globale actuelle */
        uint global_zeros = output_buffer[1];
        
        /* Si meilleur, écrire (race condition acceptable pour test) */
