...
__kernel void btc_sha256_mining(
    ...
    /* C370 FIX: Préparer message schedule pour 20 uint32 (pas 16) */
    #pragma unroll
    for (int i = 0; i < 16; i++) {
        w[i] = data[i];
    }
    ...
    w[16] = 0x80000000;  /* Bit padding après 80 bytes */
    #pragma unroll
    for (int i = 17; i < 31; i++) {
        w[i] = 0;
    }
    ...
    /* Étendre le message schedule */
    #pragma unroll
    for (int i = 32; i < 64; i++) {
        w[i] = SIG1(w[i-2]) + w[i-7] + SIG0(w[i-15]) + w[i-16];
    }
    
    /* CORRECTION: Utiliser seulement les 16 premiers pour la compression initiale */
    uint w_first[64];
    #pragma unroll
    for (int i = 0; i < 16; i++) {
        w_first[i] = data[i];
    ...
    #pragma unroll
    for (int i = 16; i < 64; i++) {
        w_first[i] = SIG1(w_first[i-2]) + w_first[i-7] + SIG0(w_first[i-15]) + w_first[i-16];
    }
    ...
    /* C370 FIX BUG #3 & #4: Format output simplifié
     * PROBLÈME: Buffer trop petit (40 bytes pour 10M work-items)
     * SOLUTION: Écrire seulement si leading_zeros > 0 (near-miss trouvé)
     * Format: output[0] = best_nonce, output[1] = best_leading_zeros
     *
     * Note: Sans atomiques, on utilise une approche simple:
     * - Chaque thread compare avec output[1]
     * - Si meilleur, écrit son résultat
     * - Race condition possible mais acceptable (on garde un bon résultat)
     */
    if (leading_zeros > 0) {
        /* Lire valeur actuelle (peut être 0 ou un autre résultat) */
        uint current_best = output_buffer[1];
        
        /* Si notre résultat est meilleur, l'écrire */
