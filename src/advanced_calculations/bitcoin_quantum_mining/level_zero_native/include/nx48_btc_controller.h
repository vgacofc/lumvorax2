 ...
 * nx48_btc_controller.h — Contrôleur NX48 autonome — Architecture C61 + C170-NX49
 *
...
/* C170-NX49-INTEGRATION : Neurone biologique atomiquement précis */
// #include "../../../neural_network/nx49_biological_neuron.h"  // C342: Commenté pour compilation standalone
...
typedef struct {
    ...
    /* ── C170-NX49-INTEGRATION : Neurone biologique pour contrôle adaptatif ── */
    // nx49_biological_neuron_t* bio_neuron;  /* Neurone pyramidal avec soma, dendrites, axon, synapses */
    void* bio_neuron;  /* Placeholder pour neurone biologique (désactivé temporairement) */
    double last_membrane_potential;         /* Dernier potentiel membrane (mV) */
    uint64_t spike_count;                   /* Nombre potentiels d'action générés */
    double bio_modulation_factor;           /* Facteur modulation biologique [-1, +1] */
} nx48_btc_state_t;
...
/* ════════════════════════════════════════════════════════════════════
 * C255V11 : ATOMIQUES PARTAGÉES GPU NATIVE (i915 DRM)
 * NX48 gère dynamiquement TOUT : batch_size, contextes, mémoire GPU
 * ════════════════════════════════════════════════════════════════════ */
extern _Atomic int      nx48_ctrl_n_threads;
extern _Atomic int      nx48_ctrl_T_hot_idx;
extern _Atomic int      nx48_ctrl_T_cold_idx;
extern _Atomic int      nx48_ctrl_gpu_active;
extern _Atomic int      nx48_ctrl_avx_level;
extern _Atomic int      nx48_ctrl_batch_size;
extern _Atomic int      nx48_ctrl_near_miss_bits;

/* C255V11 : Gestion dynamique GPU native */
extern _Atomic uint64_t nx48_ctrl_gpu_mem_available;  /* Mémoire GPU disponible (bytes) */
extern _Atomic uint64_t nx48_ctrl_gpu_mem_total;      /* Mémoire GPU totale (bytes) */
extern _Atomic int      nx48_ctrl_num_contexts;       /* Nombre contextes GPU (1, 2 ou 3) */
extern _Atomic uint32_t nx48_ctrl_batch_size_gen9;    /* Batch size Gen9 (256K départ) */
extern _Atomic int      nx48_ctrl_use_thp;            /* Utiliser THP pour buffers >2MB */
