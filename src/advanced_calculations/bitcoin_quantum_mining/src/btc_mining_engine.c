...
static void* btc_watchdog_thread(void* arg) {
    ...
    while (!atomic_load_explicit(&btc_watchdog_stop, memory_order_relaxed)) {
        ...
        if (ram_mb < 10) {
            /* NIVEAU 4 : PAUSE — RAM critique < 10MB, risque OOM killer */
            throttle = 0; divisor = 4; pause = 1;
            fprintf(stderr, "[C42-WATCHDOG] 🔴 RAM CRITIQUE: %ldMB — PAUSE totale\n", ram_mb);
        } else if (ram_mb < 200) {
            /* NIVEAU 3.5 : throttle très fort mais pas de pause */
            throttle = 50000; divisor = 8; pause = 0;
            if (tick % 3 == 0)
                printf("[C42-WATCHDOG] 🔴 RAM très basse: %ldMB — throttle 50ms batch/8\n", ram_mb);
        } else if (ram_mb < 500) {
        ...
        if (work->thread_id == 0) {
            ...
            if (elapsed_s >= 0.0 &&
                ...
                fprintf(stderr, "[C209-GPU-IMMEDIATE-ACTIVATION] GPU: %s\n",
                        eng->nx48->hw.gpu_name[0] ? eng->nx48->hw.gpu_name : "GPU-Détecté");
    ...
    for (;;) {
        ...
        if (batch_count < 5) {
            fprintf(stderr, "[C215-PIPELINE] Launching batch %"PRIu64" on buffer[%d] (nonce=0x%08X, size=%u, target=%u bits)\n",
                    batch_count, buffer_idx, nonce_start, BTC_OCL_BATCH_DEFAULT, gw->target_bits);
            fflush(stderr);
        }
...
int btc_engine_run(const btc_engine_config_t* cfg, nx48_btc_state_t* nx48) {
    ...
    if (nx48 && nx48->hw.gpu_opencl_active) {
        ...
        /* C285 : Initialiser adaptateur Gen9 pour compatibilité API OpenCL */
        ...
        /* C223-GEN9-NATIVE : Module Gen9 Native + Adaptateur initialisés — 0% OpenCL, 0% Level Zero
