...
int main(void) {
    printf("CPU BASELINE SHA-256 (sans DRM)\n");
    printf("================================\n\n");
    
    uint8_t block[80];
    memset(block, 0xAB, 80);
    
    struct timespec start, end;
    clock_gettime(CLOCK_MONOTONIC, &start);
    
    uint64_t total = 2621440;  // Même nombre que C193
    for (uint64_t i = 0; i < total; i++) {
        uint32_t* nonce = (uint32_t*)(block + 76);
        *nonce = i;
        uint8_t h1[32], h2[32];
        sha256_hash(block, 80, h1);
    ...
    printf("C193 (avec DRM): 0.65 MH/s\n");
    printf("Overhead DRM: %.1fx slowdown\n", mhs / 0.65);
