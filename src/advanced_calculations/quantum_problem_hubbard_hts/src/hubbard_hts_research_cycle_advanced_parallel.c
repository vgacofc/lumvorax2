...
#undef FOPEN_DIAG
    ...
    double pt_chi_sc[16]       = {0.0};
    {
        ...
        #define PTMC_MAX_BYTES (90LL * 1024LL * 1024LL)
        ...
        for (int i = 0; i < nprobs; ++i) {
            /* Rotation : si le fichier courant ≥ 90 MB, ouvrir une nouvelle partie */
            if (ptcsv) {
                ...
                if (pos >= PTMC_MAX_BYTES) {
                    ...
                    fprintf(stderr,
                        "[PTMC] Rotation CSV → %s (partie %d, cap 90 MB atteint)\n",
