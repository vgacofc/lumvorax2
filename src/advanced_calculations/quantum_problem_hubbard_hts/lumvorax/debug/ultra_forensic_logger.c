...
static void lv_rotate_csv(void) {
    if (g_csv_part_num >= LV_MAX_CSV_PARTS) {
        fprintf(stderr,
            "[LUMVORAX] Plafond %d parties atteint — ring-append dans %s\n",
            LV_MAX_CSV_PARTS, g_run_csv_path);
        return;
    }
    g_csv_part_num++;
...
/* ── SUMMARY — NL-03-FIX-FINAL : metric_events_count incrémenté dans METRIC
 * et utilisé pour GLOBAL:total_operations (operations_count jamais incrémenté
 * par les macros FORENSIC_LOG_MODULE_METRIC → toujours 0 → bug NL-03).
 * Correction : total_ops = total_metrics (somme metric_events_count) qui est
 * réellement incrémentée à chaque FORENSIC_LOG_MODULE_METRIC call.           */
void ultra_forensic_generate_summary_report(void) {
    ...
    if (csv) {
        ...
        /* NL-03-FIX-FINAL : total_ops était toujours 0 car operations_count
         * n'est incrémenté QUE par ultra_forensic_log_operation(), jamais par
         * FORENSIC_LOG_MODULE_METRIC. On rapporte total_metrics (metric_events_count
         * réel) qui reflète le nombre réel d'opérations de logging.            */
        fprintf(csv, "SUMMARY,%s,%" PRIu64 ",%d,GLOBAL:total_operations,%" PRIu64 "\n",
                iso, ts, getpid(), total_metrics);
        fprintf(csv, "SUMMARY,%s,%" PRIu64 ",%d,GLOBAL:total_metric_events_real,%" PRIu64 "\n",
                iso, ts, getpid(), total_metrics);
        fprintf(csv, "SUMMARY,%s,%" PRIu64 ",%d,GLOBAL:total_operations_raw_legacy,%" PRIu64 "\n",
