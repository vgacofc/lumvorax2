...
bool log_manager_archive_session(log_manager_t* manager, const char* session_id) {
    ...
    /* FIX-BC25-02: contrôle retour system() — échec non-fatal (archivage best-effort) */
    if (system(command) < 0)
        fprintf(stderr, "[LOG_MANAGER] WARN: archivage current échoué\n");
    ...
    snprintf(command, sizeof(command), 
             ...
             logs_base, session_id, archive_dir);
    if (system(command) < 0)
        fprintf(stderr, "[LOG_MANAGER] WARN: archivage modules échoué\n");
    ...
    if (system(command) < 0)
        fprintf(stderr, "[LOG_MANAGER] WARN: archivage log principal échoué\n");
