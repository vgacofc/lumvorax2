...
#define PT_MC_N_THERMALIZE     500    /* Q-NEW-03  : thermalisation avant prod.  */
...
static void pt_mc_run(const problem_t* p, uint64_t seed,
    ...
    for (int sw = 0; sw < PT_MC_N_THERMALIZE; ++sw) {
        ...
        (void)swap_acc_th; (void)swap_n_th;
    }

    /* Production — sweeps enregistrés aux points clés */
    ...
    if (out_pairing_cold) *out_pairing_cold = final_pairing_cold;
    if (out_divergence) {
...
int main(int argc, char** argv) {
    ...
    /* --- P0-C19-01 / P1-C19-01/03 / Q-NEW-03 : PT-MC 13 modules
     *     T_RATIO=50, N_SWEEPS=2000, thermalisation=500 sweeps             --- */
    ...
    double pt_pairing_cold[16] = {0.0};
    {
        ...
        for (int i = 0; i < nprobs; ++i) {
            double e_cold = 0.0, div = 0.0, p_cold = 0.0;
            ...
            fprintf(lg,
                "%06d | PT_MC problem=%s E_cold=%.6f pairing_cold=%.6f div_vs_mc=%.4f\n",
                line++, probs[i].name, e_cold, p_cold, div);
        }
        if (ptcsv) fclose(ptcsv);
    }

    /* --- P2-C19-03 : Estimation Tc par interpolation PT-MC (hubbard_hts_core)
     * Méthode : scan T ∈ {60,80,95,120,150,200,300} K, pairing(T) réplique froide.
     * Tc = T où dpairing/dT est maximal (brisure de pente = signature BKT/BCS).  --- */
    double Tc_est_global = 0.0;  /* portée étendue pour mark(&physical) après scores */
    {
        char tc_csv_path[MAX_PATH];
        pjoin(tc_csv_path, sizeof(tc_csv_path), tests, "tc_estimation_ptmc.csv");
        FILE* tccsv = fopen(tc_csv_path, "w");
        if (tccsv) fprintf(tccsv, "temp_K,E_cold_eV,pairing_cold,dpairing_dT\n");

        const double tc_temps[] = {60.0, 80.0, 95.0, 120.0, 150.0, 200.0, 300.0};
        const int    n_tc       = 7;
        double tc_pair[7] = {0.0}, tc_E[7] = {0.0};
        ...
        for (int ti = 0; ti < n_tc; ++ti) {
            ...
            double ec = 0.0, dv = 0.0, pc = 0.0;
            pt_mc_run(&ph, (uint64_t)(0xCA000000ULL + (uint64_t)ti),
        ...
        /* Dérivée numérique centree dpairing/dT, recherche du max absolu → Tc */
        double Tc_est  = tc_temps[0];
        double max_dpt = 0.0;
        for (int ti = 1; ti < n_tc - 1; ++ti) {
            double dpt = fabs((tc_pair[ti+1] - tc_pair[ti-1]) /
                              (tc_temps[ti+1] - tc_temps[ti-1]));
            if (dpt > max_dpt) { max_dpt = dpt; Tc_est = tc_temps[ti]; }
        }
        ...
        for (int ti = 0; ti < n_tc; ++ti) {
            ...
            if (ti > 0 && ti < n_tc - 1)
                dpt_i = fabs((tc_pair[ti+1] - tc_pair[ti-1]) /
                              (tc_temps[ti+1] - tc_temps[ti-1]));
            if (tccsv)
                fprintf(tccsv, "%.2f,%.10f,%.10f,%.10f\n",
                        tc_temps[ti], tc_E[ti], tc_pair[ti], dpt_i);
        }
        if (tccsv) {
            fprintf(tccsv, "# Tc_estimate_K,%.2f,max_dpairing_dT,%.10f\n", Tc_est, max_dpt);
            fclose(tccsv);
        }
        fprintf(lg, "%06d | TC_ESTIMATE hubbard_hts_core Tc=%.1f K max_dP_dT=%.6f\n",
                line++, Tc_est, max_dpt);
        FORENSIC_LOG_MODULE_METRIC("pt_mc", "Tc_estimate_K", Tc_est);
        /* Tc_est_global mis à jour — mark(&physical) appliqué après décl. scores */
    }
