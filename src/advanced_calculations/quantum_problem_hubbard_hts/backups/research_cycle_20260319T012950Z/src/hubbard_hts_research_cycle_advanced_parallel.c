...
#undef FOPEN_DIAG
    ...
    for (int i = 0; i < bn_mod; ++i) {
        ...
        p.temp_K = br->t;
        p.u_eV = br->u;
        /* BUG-FAR-EQ-C41-FIX : normaliser energy par sites (références en eV/site).
         * La référence external_module_benchmarks_v1.csv est en eV/site.
         * rr.energy_eV est l'énergie TOTALE → diviser par n_sites.                */
        double n_sites_br = (double)(p.lx * p.ly);
        sim_result_t rr = simulate_fullscale(&p, 5151 + (uint64_t)i, 129, NULL);
        /* BC-11-ADV : energy_eV est l'énergie totale → normaliser par n_sites pour eV/site */
        double model = (strcmp(br->observable, "pairing") == 0)
            ? rr.pairing_norm / (n_sites_br > 0 ? n_sites_br : 1.0)
            : rr.energy_eV   / (n_sites_br > 0 ? n_sites_br : 1.0);
