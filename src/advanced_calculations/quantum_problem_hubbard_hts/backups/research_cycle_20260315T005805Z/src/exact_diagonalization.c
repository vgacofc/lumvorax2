...
double ed_compare_mc(const ed_result_t* ed, double mc_energy_eV,
    ...
    double abs_err  = fabs(mc_energy_eV - ed->ground_energy_eV);
    double rel_err  = abs_err / fabs(ed->ground_energy_eV);

    FORENSIC_LOG_ALGO(module_name, "ed_energy_eV",    ed->ground_energy_eV);
    FORENSIC_LOG_ALGO(module_name, "mc_energy_eV",    mc_energy_eV);
    ...
    fprintf(stderr, "[ED_vs_MC] module=%s ED=%.6f eV MC=%.6f eV err=%.4f%% (%s)\n",
            module_name, ed->ground_energy_eV, mc_energy_eV, rel_err * 100.0,
