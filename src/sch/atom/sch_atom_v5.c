    ...
    for (int i = 0; i < NUM_ATOMS; i++) {
        for (int j = i + 1; j < NUM_ATOMS; j++) {
            ...
            if (d < THRESHOLD) {
                ...
                if (d < 0.12) {
                    strncpy(inv.interaction_type, "Ionic", sizeof(inv.interaction_type) - 1);
                    inv.interaction_type[sizeof(inv.interaction_type) - 1] = '\0';
                } else if (d < 0.20) {
                    strncpy(inv.interaction_type, "Hydrogen", sizeof(inv.interaction_type) - 1);
                    inv.interaction_type[sizeof(inv.interaction_type) - 1] = '\0';
                } else {
                    strncpy(inv.interaction_type, "vdW", sizeof(inv.interaction_type) - 1);
                    inv.interaction_type[sizeof(inv.interaction_type) - 1] = '\0';
                }
