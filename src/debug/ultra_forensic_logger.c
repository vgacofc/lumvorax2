...
static void csv_write_line(const char* line) {
    ...
    /* C218-FORENSIC-LIGHT : Sampling - skip N-1 appels sur N
     * Exemple: sampling_rate=50 → log 1 ligne sur 2 (50% overhead réduit)
     * Exemple: sampling_rate=1  → log 1 ligne sur 100 (99% overhead réduit)
     * IMPORTANT: Les événements critiques (INIT, MODULE_START/END, ANOMALY)
     * ne sont JAMAIS skippés (voir fonctions appelantes) */
    if (g_forensic_sampling_rate < 100) {
        uint64_t counter = __atomic_fetch_add(&g_forensic_sample_counter, 1, __ATOMIC_RELAXED);
        /* Modulo pour sampling : log seulement si counter % (100/rate) == 0
    ...
    if (g_csv_fp) {
        ...
        if (g_csv_fp) {
            fputs(line, g_csv_fp);
            g_csv_write_count++;
            /* C218-FORENSIC-LIGHT : Flush adaptatif selon sampling
...
bool ultra_forensic_logger_init(void) {
    ...
    /* C218-FORENSIC-LIGHT : Lire variable d'environnement BTC_FORENSIC_SAMPLING */
    ...
    if (env_sampling) {
        ...
        g_forensic_sampling_rate = 100;  /* Défaut : 100% = pas de sampling */
    }
    
    g_forensic_initialized = true;
    printf("[ULTRA_FORENSIC] Système de logging forensique v3.0 initialisé (sampling: %d%%)\n",
           g_forensic_sampling_rate);
    return true;
}
