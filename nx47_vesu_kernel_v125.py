...
def train_nx47_supervised(
    ...
    for neuron_id in range(max(1, cfg.meta_neurons)):
        for l1 in cfg.l1_candidates:
            for l2 in cfg.l2_candidates:
                ...
                objective = ce + 0.02 * (1.0 - sparsity) - 0.20 * stats['f1'] - 0.10 * stats['iou'] + 0.005 * reg_lambda
                rec = {
                    'neuron_id': neuron_id,
                    'l1': float(l1),
                    'l2': float(l2),
    ...
    if best is not None:
        memory.update(float(best.get('val_f1', 0.0)), float(best.get('val_f1', 0.0)))
...
def train_nx47_autonomous(features: np.ndarray, cfg: V125Config, rng: np.random.Generator, memory: NX47EvolutionMemory | None = None) -> Tuple[NX47AtomNeuron, Dict[str, Any]]:
...
def hysteresis_topology_3d(prob: np.ndarray, cfg: V125Config) -> np.ndarray:
...
class NX47V125Kernel:
    def __init__(self, root: Path = Path('/kaggle/input/competitions/vesuvius-challenge-surface-detection'), output_dir: Path = Path('/kaggle/working'), config: V125Config | None = None) -> None:
        self.version = 'NX47 V125'
        ...
        self.tmp_dir = output_dir / 'tmp_masks_v125'
        self.overlay_dir = output_dir / 'overlays_v125'
        self.submission_path = output_dir / 'submission.zip'
        self.roadmap_path = output_dir / 'v125_roadmap_realtime.json'
        self.logs_path = output_dir / 'v125_execution_logs.json'
        self.memory_path = output_dir / 'v125_memory_tracker.json'
        self.metadata_path = output_dir / 'v125_execution_metadata.json'
        self.ultra_log_path = output_dir / 'v125_ultra_authentic_360_merkle.jsonl'

        self.cfg = config or V125Config()
    ...
    def build_supervised_model(self) -> None:
        ...
        self.log('SUPERVISED_TRAIN', status='ok', train_samples=int(x_train.shape[0]), val_samples=int(x_val.shape[0]), train_info=info)
...
if __name__ == '__main__':
    cfg = V125Config(
        top_k_features=int(os.environ.get('V125_TOP_K_FEATURES', '6')),
        target_active_ratio=float(os.environ.get('V125_TARGET_ACTIVE_RATIO', '0.03')),
        full_pixel_trace=os.environ.get('V125_FULL_PIXEL_TRACE', '0') == '1',
        trace_pixel_budget=int(os.environ.get('V125_TRACE_PIXEL_BUDGET', '4000')),
        ultra_console_log=os.environ.get('V125_ULTRA_CONSOLE_LOG', '1') == '1',
        ultra_step_log=os.environ.get('V125_ULTRA_STEP_LOG', '1') == '1',
        ultra_bit_trace_arrays=os.environ.get('V125_ULTRA_BIT_TRACE_ARRAYS', '1') == '1',
        ultra_bit_trace_limit=int(os.environ.get('V125_ULTRA_BIT_TRACE_LIMIT', '64')),
        meta_neurons=int(os.environ.get('V125_META_NEURONS', '3')),
        run_simulation_100=os.environ.get('V125_RUN_SIMULATION_100', '0') == '1',
        simulation_export_curve=os.environ.get('V125_SIMULATION_EXPORT_CURVE', '1') == '1',
        supervised_train=os.environ.get('V125_SUPERVISED_TRAIN', '1') == '1',
        max_train_volumes=int(os.environ.get('V125_MAX_TRAIN_VOLUMES', '24')),
        max_val_volumes=int(os.environ.get('V125_MAX_VAL_VOLUMES', '8')),
        max_samples_per_volume=int(os.environ.get('V125_MAX_SAMPLES_PER_VOLUME', '40000')),
        pos_neg_ratio=float(os.environ.get('V125_POS_NEG_RATIO', '1.0')),
        golden_nonce_topk=int(os.environ.get('V125_GOLDEN_NONCE_TOPK', '11')),
    )
    kernel = NX47V125Kernel(
