...
def hysteresis_topology_3d(prob: np.ndarray, cfg: V116Config) -> np.ndarray:
...
class NX47V116AtomKernel:
    def __init__(
        ...
        self.version = "NX47 V116 ATOM"
        self.root = self._resolve_root(root)
        self.test_dir = self.root / "test_images"
        self.output_dir = output_dir
        self.tmp_dir = output_dir / "tmp_masks_v116"
        self.overlay_dir = output_dir / "overlays_v116"
        self.submission_path = output_dir / "submission_v116.zip"
        self.roadmap_path = output_dir / "v116_roadmap_realtime.json"
        self.logs_path = output_dir / "v116_execution_logs.json"
        self.memory_path = output_dir / "v116_memory_tracker.json"
        self.metadata_path = output_dir / "v116_execution_metadata.json"

        self.cfg = config or V116Config()
...
if __name__ == "__main__":
    cfg = V116Config(
        top_k_features=int(os.environ.get("V116_TOP_K_FEATURES", "6")),
        target_active_ratio=float(os.environ.get("V116_TARGET_ACTIVE_RATIO", "0.03")),
        full_pixel_trace=os.environ.get("V116_FULL_PIXEL_TRACE", "0") == "1",
        trace_pixel_budget=int(os.environ.get("V116_TRACE_PIXEL_BUDGET", "4000")),
    )
    kernel = NX47V116AtomKernel(
