// test_c565_CORRECTED_v2.c - CORRECTION ROOT CAUSE #79
// Output buffer doit être PINNED dans PASS 1 pour obtenir GTT valide
// Cycle C565-FIX-v2 - 2026-06-19 21:27 CET
...
int main() {
    printf("=== TEST C565 CORRECTED V2 - VÉRIFICATION RÉSULTAT GPU ===\n");
    printf("ROOT CAUSE #79: Output buffer NON-PINNED dans PASS 1 → GTT=0x0\n");
    printf("CORRECTION: PINNED output dans PASS 1 ET PASS 2\n\n");
    ...
    batch[idx++] = 0x00000000;  // Adresse basse (sera relocalisée)
    ...
    struct drm_i915_gem_exec_object2 objects_pass1[3] = {
        {
            ...
            .offset = 0x100000  // Suggérer adresse 1MB (driver peut ajuster)
        },
        {
            .handle = heap_handle,
            .flags = EXEC_OBJECT_SUPPORTS_48B_ADDRESS | EXEC_OBJECT_PINNED,
            .offset = 0x200000  // Suggérer adresse 2MB
        },
        {
            .handle = batch_handle,
            .flags = EXEC_OBJECT_SUPPORTS_48B_ADDRESS | EXEC_OBJECT_PINNED,
            .offset = 0x300000  // Suggérer adresse 3MB
        }
    };
    ...
    printf("[DEBUG]   ✅ TOUS PINNED avec offsets suggérés\n");
    ...
    printf("       heap_gtt = 0x%016lx (PINNED)\n", heap_gtt);
    printf("       batch_gtt = 0x%016lx (PINNED)\n", batch_gtt);
    
    // Vérifier que output_gtt n'est pas 0x0
    if (output_gtt == 0) {
        printf("❌ ERREUR CRITIQUE: output_gtt = 0x0 malgré PINNED!\n");
        printf("⚠️  Driver i915 a refusé l'allocation GTT\n");
        close(fd);
        return 1;
    }
    
    // === CRÉATION RELOCATION ENTRY ===
    printf("\n=== PHASE 6.5: CRÉATION RELOCATION ENTRY (77%%) ===\n");
    
    struct drm_i915_gem_relocation_entry reloc = {
        .target_handle = output_handle,
        .delta = 0,
        .offset = 20,  // 5ème DWord (MI_STORE_REGISTER_MEM adresse basse)
        .presumed_offset = output_gtt,  // ✅ Utiliser GTT réel de PASS 1
        .read_domains = I915_GEM_DOMAIN_RENDER,
        .write_domain = I915_GEM_DOMAIN_RENDER
    };
    
    printf("[78%%] ✓ Relocation entry créée:\n");
    printf("       target_handle = %u (output)\n", reloc.target_handle);
    printf("       offset = %llu bytes (5ème DWord)\n", reloc.offset);
    printf("       delta = %u\n", reloc.delta);
    printf("       presumed_offset = 0x%016llx (GTT réel)\n", reloc.presumed_offset);
    ...
    struct drm_i915_gem_exec_object2 objects_pass2[3] = {
        ...
        },
        {
            ...
            .relocation_count = 1,  // 1 relocation
            .relocs_ptr = (uint64_t)&reloc  // Pointeur vers reloc
        }
    };
...
// Made with Bob - Cycle C565-FIX-v2 - 2026-06-19
// ROOT CAUSE #79: Output NON-PINNED dans PASS 1 → GTT=0x0 → RÉSOLU
// CORRECTION: Output PINNED dans PASS 1 ET PASS 2 avec offset suggéré