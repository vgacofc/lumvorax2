...
def hysteresis_topology_3d(prob: np.ndarray, cfg: V117Config) -> np.ndarray:
...
class NX47V117Kernel:
    def __init__(
        self,
        root: Path = Path("/kaggle/input/competitions/vesuvius-challenge-surface-detection"),
        output_dir: Path = Path("/kaggle/working"),
        config: V117Config | None = None,
    ) -> None:
        self.version = "NX47 V117"
        self.root = self._resolve_root(root)
        self.test_dir = self.root / "test_images"
        self.output_dir = output_dir
        self.tmp_dir = output_dir / "tmp_masks_v117"
        self.overlay_dir = output_dir / "overlays_v117"
        self.submission_path = output_dir / "submission.zip"
        self.roadmap_path = output_dir / "v117_roadmap_realtime.json"
        self.logs_path = output_dir / "v117_execution_logs.json"
        self.memory_path = output_dir / "v117_memory_tracker.json"
        self.metadata_path = output_dir / "v117_execution_metadata.json"

        self.cfg = config or V117Config()
...
if __name__ == "__main__":
    cfg = V117Config(
        top_k_features=int(os.environ.get("V117_TOP_K_FEATURES", "6")),
        target_active_ratio=float(os.environ.get("V117_TARGET_ACTIVE_RATIO", "0.03")),
        full_pixel_trace=os.environ.get("V117_FULL_PIXEL_TRACE", "0") == "1",
        trace_pixel_budget=int(os.environ.get("V117_TRACE_PIXEL_BUDGET", "4000")),
    )
    kernel = NX47V117Kernel(
