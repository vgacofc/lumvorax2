...
class V130Config:
    ...
    # V130 2.5D U-Net competitive path
...
def _extract_2p5d_patches(vol: np.ndarray, lbl2d: np.ndarray, cfg: V130Config, rng: np.random.Generator) -> Tuple[np.ndarray, np.ndarray]:
    z, h, w = vol.shape
...
def train_unet_25d_supervised(train_x: np.ndarray, train_y: np.ndarray, val_x: np.ndarray, val_y: np.ndarray, cfg: V130Config, rng: np.random.Generator) -> Dict[str, Any]:
...
def train_nx47_autonomous(features: np.ndarray, cfg: V130Config, rng: np.random.Generator, memory: NX47EvolutionMemory | None = None) -> Tuple[NX47AtomNeuron, Dict[str, Any]]:
...
def hysteresis_topology_3d(prob: np.ndarray, cfg: V130Config) -> np.ndarray:
...
class NX47V130Kernel:
    def __init__(self, root: Path = Path('/kaggle/input/competitions/vesuvius-challenge-surface-detection'), output_dir: Path = Path('/kaggle/working'), config: V130Config | None = None) -> None:
        self.version = 'NX47 V130'
        ...
        self.tmp_dir = output_dir / 'tmp_masks_v130'
        self.overlay_dir = output_dir / 'overlays_v130'
        self.submission_path = output_dir / 'submission.zip'
        self.roadmap_path = output_dir / 'v130_roadmap_realtime.json'
        self.logs_path = output_dir / 'v130_execution_logs.json'
        self.memory_path = output_dir / 'v130_memory_tracker.json'
        self.metadata_path = output_dir / 'v130_execution_metadata.json'
        self.ultra_log_path = output_dir / 'v130_ultra_authentic_360_merkle.jsonl'

        self.cfg = config or V130Config()
...
if __name__ == '__main__':
    cfg = V130Config(
        top_k_features=int(os.environ.get('V130_TOP_K_FEATURES', '6')),
        target_active_ratio=float(os.environ.get('V130_TARGET_ACTIVE_RATIO', '0.03')),
        full_pixel_trace=os.environ.get('V130_FULL_PIXEL_TRACE', '0') == '1',
        trace_pixel_budget=int(os.environ.get('V130_TRACE_PIXEL_BUDGET', '4000')),
        ultra_console_log=os.environ.get('V130_ULTRA_CONSOLE_LOG', '1') == '1',
        ultra_step_log=os.environ.get('V130_ULTRA_STEP_LOG', '1') == '1',
        ultra_bit_trace_arrays=os.environ.get('V130_ULTRA_BIT_TRACE_ARRAYS', '1') == '1',
        ultra_bit_trace_limit=int(os.environ.get('V130_ULTRA_BIT_TRACE_LIMIT', '64')),
        meta_neurons=int(os.environ.get('V130_META_NEURONS', '3')),
        run_simulation_100=os.environ.get('V130_RUN_SIMULATION_100', '0') == '1',
        simulation_export_curve=os.environ.get('V130_SIMULATION_EXPORT_CURVE', '1') == '1',
        supervised_train=os.environ.get('V130_SUPERVISED_TRAIN', '1') == '1',
        max_train_volumes=int(os.environ.get('V130_MAX_TRAIN_VOLUMES', '24')),
        max_val_volumes=int(os.environ.get('V130_MAX_VAL_VOLUMES', '8')),
        max_samples_per_volume=int(os.environ.get('V130_MAX_SAMPLES_PER_VOLUME', '40000')),
        pos_neg_ratio=float(os.environ.get('V130_POS_NEG_RATIO', '1.0')),
        golden_nonce_topk=int(os.environ.get('V130_GOLDEN_NONCE_TOPK', '11')),
        supervised_epochs=int(os.environ.get('V130_SUPERVISED_EPOCHS', '3')),
        fbeta_beta=float(os.environ.get('V130_F_BETA', '0.5')),
        use_unet_25d=os.environ.get('V130_USE_UNET_25D', '1') == '1',
        unet_in_slices=int(os.environ.get('V130_UNET_IN_SLICES', '7')),
        unet_base_channels=int(os.environ.get('V130_UNET_BASE_CHANNELS', '24')),
        patch_size=int(os.environ.get('V130_PATCH_SIZE', '128')),
        patch_stride=int(os.environ.get('V130_PATCH_STRIDE', '64')),
        unet_epochs=int(os.environ.get('V130_UNET_EPOCHS', '2')),
        unet_lr=float(os.environ.get('V130_UNET_LR', '0.001')),
        unet_batch_size=int(os.environ.get('V130_UNET_BATCH_SIZE', '8')),
        export_logit_audit=os.environ.get('V130_EXPORT_LOGIT_AUDIT', '1') == '1',
        logit_hist_bins=int(os.environ.get('V130_LOGIT_HIST_BINS', '20')),
    )
    kernel = NX47V130Kernel(
