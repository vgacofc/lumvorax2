# ================================================================
# LUMVORAX DEPENDENCY 360 VALIDATION (KAGGLE SINGLE CELL - V2)
# ================================================================
# Self-contained: no import from local helper scripts.
# Paste this file content directly into one Kaggle notebook code cell.

from __future__ import annotations
import json, os, platform, re, shutil, struct, subprocess, sys, tempfile, time, importlib
...
def install_offline_if_missing(pkg: str) -> Dict[str, Any]:
    ...
    for root in KAGGLE_DEP_PATHS:
        ...
        try:
            if wheel.exists():
                subprocess.check_call([py, '-m', 'pip', 'install', '--no-index', str(wheel)])
            else:
                subprocess.check_call([py, '-m', 'pip', 'install', '--no-index', f'--find-links={root}', pkg])
            if pkg_available(pkg):
                return {"package": pkg, "status": "installed", "root": str(root)}
        except Exception as exc:
            last = str(exc)
            continue
    return {"package": pkg, "status": "missing", "error": locals().get('last', 'not found in kaggle dependency roots')}
...
def tiff_lum_roundtrip_test() -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        return {
            'status': 'ok', 'shape': [int(x) for x in restored.shape], 'dtype': str(restored.dtype),
            'global_min': float(restored.min()), 'global_max': float(restored.max()), 'global_mean': float(restored.mean()),
            'global_nonzero_pct': float((restored > 0).mean() * 100.0), 'slice_logs': slice_logs,
            'roundtrip_ok': bool(np.allclose(arr3d, restored, atol=0.0)),
            'tiff_sha512': sha512(tif_path.read_bytes()).hexdigest(), 'lum_sha512': sha512(lum_path.read_bytes()).hexdigest(),
        }
...
def main() -> None:
    ...
    report = {
        'report_name': 'lumvorax_dependency_360_unified_single_cell_v2',
        'timestamp_ns': time.time_ns(),
        'runtime': {'python': sys.version, 'platform': platform.platform(), 'cwd': str(Path.cwd()), 'is_kaggle': Path('/kaggle').exists()},
    ...
    out = Path('/kaggle/working/lumvorax_360_validation_report_v2.json')
    if not out.parent.exists():
        out = Path('lumvorax_360_validation_report_v2.json')
